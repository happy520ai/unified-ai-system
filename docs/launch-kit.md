...
Unified AI System is an open, local-first AI gateway for multi-model routing,
...
The terminal is the primary interface: `pnpm gateway demo` proves the isolated
path, while `serve`, `status`, `chat`, and `doctor` provide a practical local
...
终端是当前主入口：`pnpm gateway demo` 用于验证隔离链路，`serve`、`status`、
`chat` 和 `doctor` 组成可持续使用的本地工作流。当网关可能使用真实 Provider
时，Chat 会默认拒绝发送，只有为本次命令显式授权后才会继续。
...
A clean clone runs without credentials using a deterministic fake provider.
The source checkout includes a terminal CLI for demo, startup, status, chat,
and diagnostics. Real provider calls are explicit opt-in. The project is an
early public engineering preview, and I would value feedback on the
architecture, developer experience, and governance model.
```
...
Terminal CLI: demo, serve, status, chat, and doctor.
