...
> 已审代码提交：`5ff8a0b6d6c5ba48105dc4820c2811a332b7948f`（外部不可逆效果 fence：`4fa6001d`；PostgreSQL CI 覆盖闭环：`5ff8a0b6`；Provider dispatch：`725c1ab5`；间接 Provider sink 收口：`63569228`；首方 client/legacy self-call：`9163a642`；隔离测试编排：`492a7d0a`；运行时：`3adb9fe3`；A2A 原子终态：`0eeb2aa2`/`46da8708`）
...
| 企业生产候选 | 部分达到 | 身份、租户、预算、审计、可观测性已有；PostgreSQL claim/中央 Workforce queue+result+审批+生命周期、usage/audit、A2A task state+execution lease+原子终态、结构化对账、真实 Provider 调度墓碑，以及 built-in Git/shell、飞书/企微/告警 Webhook 的独立 durable 外部效果墓碑已获 E3；但运行中角色不能跨进程恢复，远端 exactly-once、通用 MCP/OpenAPI/custom sink、HA/DR、provider-authenticated statement 和真实提供商阶段证据不足。 |
...
| 项目 | 数量/状态 |
| --- | --- |
| Git 跟踪文件 | 1,894（包含本报告，审计统计排除本地 `.mcp.json` 用户改动） |
| JS/TS/ESM 源文件 | 1,613 |
| 测试文件 | 346 |
| `docs/` Markdown/HTML | 99 |
| pnpm 工作区项目 | 20（含根项目） |
| PR 相对 `origin/master` 变化文件 | 443（包含本报告） |
| 已审分支相对 `origin/master` | 本报告提交后 98 个提交领先；最新代码/CI 证据为 `5ff8a0b6` |
...
| A2A v1.0 | 可验证 Agent Card/JWKS、JSON-RPC 任务、取消、Workforce 模式，以及有界 memory/同主机 SQLite/跨主机 PostgreSQL 任务与 fenced execution lease | 其他智能体可验证身份、发现并调用网关；多副本共享任务生命周期，同 tenant/owner/task 仅一个有效执行者；completed/failed/canceled 与当前 fence 在一个事务提交，跨副本无本地 event bus 也可取消 | E3；真实 PostgreSQL 独立连接池通过；TaskStore 终态已原子消费 fence，Provider 与已覆盖的 Git/shell/Webhook 会先写独立墓碑并复核 claim，但远端副作用仍不能撤回或 exactly-once，通用 MCP/OpenAPI sink 与重叠签名轮换也未完成 |
...
| Forge/Workforce | 受限、可取消、资源感知的编码/多角色执行；真实 HTTP 审批/状态/取消；PostgreSQL claim、中央 queue/result/审批/lifecycle 与原子终态 fence；LLM 调用强制使用请求绑定的核心网关适配器 | 把执行置于网关预算、租户、权限、调度去重和审计边界；多副本原子消费同一审批、共享状态/结果并可远程取消，过期 owner 由更高 fence 接管，旧 token 不能提交数据库终态；built-in Git/PR/任意 shell 在 sink 前复核 claim | E3；修复 Forge 缓存首请求代理的身份/取消上下文滞留，并拒绝未带治理标记的 Workforce Provider 适配器；但崩溃中的角色调用栈不能重建，远端 Provider/Git/Webhook 接受与 Workforce fence 仍非同一原子事务，通用 MCP/OpenAPI/custom sink 未普遍消费 fence |
...
| AUD-29 | 高 | timeout 的外层 `Promise.race` 先返回，但 DAG 内部的 abort race 也可能先于实际 provider Promise 结束；随后删除 worktree 会让仍运行的底层任务面对已消失目录，旧 Forge 测试只比对源码字符串而未发现此竞态 | 取消后进入有界 drain，先等实际 role Promise 收敛；若 provider 在 `WORKFORCE_ABORT_DRAIN_TIMEOUT_MS` 内退出才允许清理，拒绝退出则标记 `execution_quiescence_unconfirmed` 并保留隔离 worktree；外层同时保留 timeout 原因与底层 settlement 错误；旧断言改为当前取消/收敛契约并增加行为测试 | E2/E3；延迟 50ms 响应 abort 的 provider 验证“先收敛、后清理”，永久忽略 abort 的 provider 验证“超时、工作树保留、remove 不调用”；本地 Forge 2,700/2,700、当前网关主要 Vitest 1,413 passed，远端完整门通过 |
...
在本轮已审范围和现有自动化证据内，**没有仍然已知且未处置的 P0 代码级缺陷**。仍知的 P1 是通用 MCP/OpenAPI/未声明 custom sink 尚未普遍强制外部效果 fence，以及远端系统不参与本地事务；它们在下节保留为阻断，不能用本轮 built-in 收口替代。
...
| 生产阻断 | Workforce 的 claim、queue/result、审批和 lifecycle 已中央化，数据库终态已原子消费 fence；Provider 调用和 built-in Git/PR/任意 shell、飞书/企微/告警 Webhook 已有独立 durable 墓碑与提交前 claim 复核，但运行中的角色调用栈/证据会话不能在副本崩溃后重建，远端接受不与本地 fence 同事务，通用 MCP/OpenAPI/未声明 custom sink 也未普遍强制 fence | 跨副本授权、状态、取消、数据库终态和已覆盖 sink 的 TTL 内重复尝试防护已经闭环；进程崩溃仍只能由租约恢复/重新执行，已发出的远端副作用不能撤回，不能声称 durable resume 或端到端 exactly-once | 可重建执行状态机或明确幂等重放契约、中央/对象化证据、Provider/外部系统侧 idempotency 或可认证对账、覆盖 MCP/OpenAPI/custom sink、数据库故障转移/分区/split-brain 测试 |
...
| P1 能力差距 | A2A 已支持稳定 Ed25519/JWKS、memory/SQLite/PostgreSQL 任务、分布式 execution lease/fence 和原子 TaskStore 终态；Provider dispatch、built-in Git/PR/任意 shell 和现有 Webhook 已能阻断同 key 重放并复核 Workforce claim，但 fence 尚不能原子传递给远端，通用 MCP/OpenAPI/custom sink 也未统一纳入，且缺重叠多签名轮换 | 数据库内 revoke/commit、覆盖 sink 的墓碑与网关侧重复外呼风险已分别关闭；它们不是同一远端事务，最终网络 TOCTOU 和未知结果仍存在，不能声称端到端 exactly-once | 让剩余不可逆 sink 在提交边界拒绝陈旧 fence，优先为 MCP/OpenAPI 工具声明副作用类型；推动 Provider/外部系统消费 idempotency/fence 或完成可认证对账，做数据库故障转移/分区测试，并完成重叠轮换演练 |
...
| `pnpm test` | 当前完整命令通过；Forge 2,700/2,700；网关 Node 100/100、隔离解析器 10/10、主要 Vitest 1,413 passed/26 conditional skipped；shared SDK 18/18、Agent Console 17/17、MCP/其余工作区套件通过。本 tranche 首次根门在 Forge 2,698/2,700 被两条仍假设远端 Git 默认注册/旧 shell 源码窗口的契约测试正确阻断；更新断言后聚焦 59/59、完整 Forge 2,700/2,700 和根门通过。更早的隔离 parser OS 终止历史仍保留，不把复跑替代失败历史 |
...
| `pnpm check:public` | 通过；1,894 个 tracked/candidate、1,834 个文本文件，0 issue codes；工作区 `.mcp.json` 有用户改动时从 Git 提交内容审计，未读取其本地内容 |
...
| 门 | 结果 | 可复核链接 |
| --- | --- | --- |
| 完整 `quality` | 已审代码/CI 基线 `5ff8a0b6` 一次通过；6 分 58 秒，quality score 270，所有 required artifact parity 通过 | [Run 32809195694](https://github.com/happy520ai/unified-ai-system/actions/runs/32809195694) |
...
| 开环 soak/背压 | v3 通过；warmup 5/5（p95 40.96ms）；持续 500/500、0 错误、p95 5.97ms、scheduler lag p95 1.03ms；突发 84 accepted/172 controlled 503；恢复与 8/8 中断通过 | Run `32809195694`；`32808063876` attempt 1 的 339 个 503+20 transport timeout/p95 4,601.18ms 与 attempt 2 恢复历史均保留，不写成长时稳定性证明 |
| SLO | 非流式 80/80、p95 36.22ms/p99 37.13ms；流式 80/80、首内容 p95 41.55ms、总响应 p95 48.92ms；0 错误，故障隔离后恢复 | Run `32809195694` |
| 资源稳定性 soak | v2 通过；1,200/1,200、arrival 1.00、0 错误、25/25 scrape；heap +9.77MiB、RSS +12.19MiB、event-loop p99 bound 22.27ms | 同一 quality run；v1 客户端自限失败 `32786355822` 保留，前一道 open-loop 仍单独要求 100 RPS/0 drop |
...
| 插件扫描 | HOL scan 与独立 plugin-scanner 均通过 | [Run 32809195709](https://github.com/happy520ai/unified-ai-system/actions/runs/32809195709)、[PR #115 checks](https://github.com/happy520ai/unified-ai-system/pull/115/checks) |
...
| 协议与智能体治理 | OpenAI/Anthropic/Gemini + MCP + A2A + reverse MCP + Forge/Workforce；A2A 已有稳定签名/JWKS、跨主机 PostgreSQL 状态、fenced execution lease 与原子 TaskStore 终态；Provider 和现有 built-in Git/shell/Webhook 有独立 durable 墓碑 | [MCP `2026-07-28`](https://github.com/modelcontextprotocol/modelcontextprotocol/blob/main/blog/content/posts/2026-07-28-spec-ga/index.md) 进入无会话现代时代；[A2A v1.0](https://github.com/a2aproject/A2A/blob/main/docs/announcing-1.0.md) 强调稳定、多租户与可签名 Agent Card | **项目最有机会领先的维度**；通用 MCP/OpenAPI/custom sink fencing、远端原子性、签名轮换和故障转移证据仍需补齐 |
...
2. **补生产闭环**：把 Workforce 运行状态改成可重建/幂等重放并中央化证据，把当前 Provider、built-in Git/shell/Webhook 的 fence 扩展到通用 MCP/OpenAPI/custom sink，并补远端可认证对账、provider-authenticated statement ingestion/持久历史/外部 WORM、签名轮换、真实 provider staging、24 小时 soak、DR 与独立渗透测试。
