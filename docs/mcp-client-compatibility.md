...
| Generic MCP Streamable HTTP host | Run `pnpm mcp:http` and configure `http://127.0.0.1:3210/mcp`. | List the fifteen tools, then call `gateway_health` and `gateway_readiness`. | The source endpoint is protocol-tested. A named client is not certified until a real client report records it. |
| Node MCP SDK test host | Run `pnpm verify:mcp`. | The `@modelcontextprotocol/client` harness tests stdio and Streamable HTTP, lists the fifteen tools, calls governed tools, checks HTTP access controls, and closes the managed gateways. | This is real protocol integration evidence covered by the source tests. It does not certify a third-party client UI. |
