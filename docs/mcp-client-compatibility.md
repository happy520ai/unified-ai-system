...
- Image: `ghcr.io/happy520ai/unified-ai-system/mcp-server:0.8.0`
...
- Registry: [official MCP Registry v0.8.0](https://registry.modelcontextprotocol.io/v0.1/servers/io.github.happy520ai%2Funified-ai-system/versions/0.8.0)
...
| Generic MCP stdio host | Add the `mcpServers.unified-ai-system` entry from [generic client configuration](mcp-generic-client.md). | Restart the host, confirm the fifteen tools, call `gateway_health`, then `gateway_readiness`. | The JSON configuration and MCP server path are covered by the repository's provider-free verification. Re-verified 2026-09-26 at `9ee4604c` (Node v25.8.1, Windows): a raw JSON-RPC client negotiated `2025-06-18`, and `tools/list` returned all fifteen names - the same set the published `0.8.0` image carries - with no provider call. `initialize` was answered after 7.5-8.5s, which is what #168 is about. |
| Generic MCP Streamable HTTP host | Run `pnpm mcp:http` and configure `http://127.0.0.1:3210/mcp`. | List the fifteen tools, then call `gateway_health` and `gateway_readiness`. | The source endpoint is protocol-tested, re-run 2026-09-26 at `9ee4604c`: the official client harness passed 16/16 including the Streamable HTTP tool-exposure and invalid-access cases, which assert the fifteen names by set. A named client is not certified until a real client report records it. |
| MCP Inspector `2.8.0` (the protocol's own reference client) | `npx -y @modelcontextprotocol/inspector --cli node packages/mcp-server/src/index.js --method tools/list` | Let the inspector spawn the server over stdio and ask it for its tool list. | Ran 2026-09-26 against master `508579ec` (npm 11.11.0, Node v25.8.1, Windows): **15 tools discovered**, identical as a set to the published `0.8.0` image roster - nothing extra, nothing missing - and 13 of the 15 carry `readOnlyHint: true`. The banner the client read from the server's stderr says "real providers disabled". A reference client proves protocol interoperability; it does not prove that a named application's configuration surface works. |
| Node MCP SDK test host | Run `pnpm verify:mcp`. | The `@modelcontextprotocol/client` harness tests stdio and Streamable HTTP, lists the fifteen tools, calls governed tools, checks HTTP access controls, and closes the managed gateways. | This is real protocol integration evidence covered by the source tests - `@modelcontextprotocol/client` `2.0.0`, `pnpm verify:mcp` 16/16 on 2026-09-26 at `9ee4604c`. It does not certify a third-party client UI. |
...
1. Confirm the host loaded the server and lists the expected fifteen tools.
