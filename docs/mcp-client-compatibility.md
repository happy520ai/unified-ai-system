...
| Codex | `codex mcp add unified-ai-system -- docker run --rm -i ghcr.io/happy520ai/unified-ai-system/mcp-server:0.4.9` | Run `codex mcp get unified-ai-system --json`, restart Codex, then use `/mcp verbose`. | Automated source-tree profile verified the official Codex App Server `0.147.0`, all nine tools, a provider-free enhancement call, and cleanup without a model turn. |
| VS Code | Configure the source MCP server in an isolated VS Code profile. | Inspect `vscode.lm.tools`, then invoke `gateway_prompt_enhance` through `vscode.lm.invokeTool`. | VS Code `1.118.1` Extension Host discovered all nine tools, invoked enhancement, made no model/provider request, and cleaned up. |
| Claude Code | Install the pinned host profile from [client runtime certification](client-runtime-certification.md). | The verifier runs Claude Code `mcp add` and `mcp list`. | Claude Code `2.1.227` issued the real MCP handshake and `tools/list`, discovered nine tools, made no model/provider call, and cleaned up. |
| Gemini CLI | Install the pinned host profile from [client runtime certification](client-runtime-certification.md). | The verifier runs `mcp add`, the connection probe, and a minimal ACP initialization. | Gemini CLI `0.54.4` used its real MCP client to discover nine tools without a prompt or model call; cleanup is verified. |
| OpenCode CLI | Install the pinned host profile from [client runtime certification](client-runtime-certification.md). | The verifier runs `opencode --pure mcp list` under isolated inline configuration. | OpenCode `1.18.16` discovered nine tools, loaded no external plugin/configuration, made no model/provider call, and cleaned up. |
| Cursor Agent CLI | Configure `.cursor/mcp.json`, then use the pinned host profile from [client runtime certification](client-runtime-certification.md). | The verifier runs `mcp enable` and `mcp list-tools unified-ai-system`. | Cursor Agent CLI `2026.08.04-aaa8809` negotiated MCP `2025-11-25`, discovered nine tools without account credentials or a model request, and cleaned up. |
| Cline CLI | `cline mcp install unified-ai-system --yes --json -- docker run --rm -i ghcr.io/happy520ai/unified-ai-system/mcp-server:0.4.9` | Run the pinned isolated profile from [client runtime certification](client-runtime-certification.md). | Cline `3.0.52` discovered nine tools and called only read-only `gateway_health` through a local fake model; no real provider was enabled or called, and cleanup is verified. |
| Continue CLI | Declare the server in local `config.yaml`, or run the pinned host profile from [client runtime certification](client-runtime-certification.md). | The verifier launches `cn --config ... -p` against the local fake model. | Continue `1.5.47` negotiated MCP `2025-11-25`, discovered nine tools, called only read-only `gateway_health`, made no real-provider attempt, and cleaned up. |
