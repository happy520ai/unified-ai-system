...
export function lookupCache(input = {}) {
  const cacheKey = typeof input === "string" ? input : input.cacheKey;
...
export function writeCacheRecord(input = {}, responseValue, metadata = {}) {
  const recordInput = typeof input === "string"
    ...
    : input;
  const policy = createResponseCachePolicy(recordInput);
  const safety = inspectCacheSafety(recordInput);
  if (!policy.cacheable || !safety.cacheEligible) {
    return auditResult("reject", {
      cacheDecision: "reject",
      cacheHitType: "no_cache",
      cacheKey: recordInput.cacheKey,
...
export function readCacheSummary() {
  return readJson(paths.summary, createSummary(readJson(paths.index, {})));
}
