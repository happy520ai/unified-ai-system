...
const SECRET_PATTERNS = [
  { marker: "openai_or_compatible_key", regex: /\bsk-(?!test|example|placeholder|redacted|xxxx)[A-Za-z0-9_-]{20,}\b/gi },
  { marker: "nvidia_key", regex: /\bnvapi-(?!test|example|placeholder|redacted|xxxx)[A-Za-z0-9_-]{20,}\b/gi },
...
const KNOWN_SAFE_PLACEHOLDER_FILES = new Set([
...
export function scanForPlaintextSecrets({ repoRoot, files }) {
  const findings = [];
  for (const file of files) {
    ...
    for (const pattern of SECRET_PATTERNS) {
      const matches = [...text.matchAll(pattern.regex)]
        .filter((match) => !isKnownSafeSecretFixture(file, text, match));
      ...
      findings.push({
        ...
        redacted: true,
        repair: "manual_required_if_real_secret",
      });
    }
  }
  return {
    ...
    findings,
    scope: "docs/evidence/ui/fixtures/source/package-scripts",
    plaintextValuesRecorded: false,
  };
}

function isKnownSafeSecretFixture(file, text, match) {
  ...
  if (!KNOWN_SAFE_PLACEHOLDER_FILES.has(file)) return false;
