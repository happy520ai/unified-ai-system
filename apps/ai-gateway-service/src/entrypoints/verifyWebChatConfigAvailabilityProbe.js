...
const PHASE = "phase-76e-web-chat-config-availability-probe";
const __dirname = dirname(fileURLToPath(import.meta.url));
const repoRoot = resolve(__dirname, "../../../..");
const evidenceDir = resolve(repoRoot, "apps/ai-gateway-service/evidence");
const evidenceJsonPath = resolve(evidenceDir, "phase-76e-web-chat-config-availability-probe.json");
const evidenceMdPath = resolve(evidenceDir, "phase-76e-web-chat-config-availability-probe.md");
const evidencePngPath = resolve(evidenceDir, "phase-76e-web-chat-config-availability-probe.png");
...
try {
  ...
  browserProfileDir = await mkdtemp(resolve(tmpdir(), "phase76e-browser-profile-"));
  ...
  try {
    ...
    evidence = {
      ...
      screenshot: {
        path: "apps/ai-gateway-service/evidence/phase-76e-web-chat-config-availability-probe.png",
      ...
      safety: {
        ...
        providerAvailabilityProbe: true,
      ...
      conclusion: passed ? "web-chat-config-availability-probe-connected" : "web-chat-config-availability-probe-not-connected",
    };
  } finally {
    await closeCdpSilently(cdp);
  }

  await writeVerifyWebChatConfigAvailabilityProbeEvidence(evidence);
  ...
  evidence = {
    ...
    conclusion: "web-chat-config-availability-probe-not-connected",
  };
  await writeVerifyWebChatConfigAvailabilityProbeEvidence(evidence);
  console.log(JSON.stringify(evidence, null, 2));
  process.exitCode = 1;
...
async function installFetchRecorder(cdp) {
  await cdp.evaluate(`(() => {
    window.__phase76eFetches = [];
    const originalFetch = window.fetch.bind(window);
    window.fetch = async (...args) => {
      const path = String(args[0] || "");
      window.__phase76eFetches.push(path);
...
async function readState(cdp) {
  return cdp.evaluate(`(() => {
    ...
    return {
      ...
      probeButtonPresent: Boolean(document.querySelector("[data-command-action='probe-provider']")),
      ...
      fetches: window.__phase76eFetches || [],
    };
  })()`);
}
...
async function writeVerifyWebChatConfigAvailabilityProbeEvidence(body) {
...
function createEvidenceMarkdown(body) {
  return `# Phase 76E Web Chat Config Availability Probe Evidence
...
- Probe button present: ${body.ui?.finalState?.probeButtonPresent}
