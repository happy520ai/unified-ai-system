...
try {
  ...
  await writeVerifyWebChatStatusFeedbackEvidence(evidence);
...
async function writeVerifyWebChatStatusFeedbackEvidence(body) {
...
function createEvidenceMarkdown(body) {
  return `# Phase 67A Web Chat Status Feedback Evidence
...
- Connecting status: ${body.ui?.afterConnecting?.latestAssistantStatus ?? "n/a"}
- Knowledge status: ${body.ui?.afterKnowledge?.latestAssistantStatus ?? "n/a"}
- Generating status: ${body.ui?.afterGenerating?.latestAssistantStatus ?? "n/a"}
- Done status: ${body.ui?.afterDone?.latestAssistantStatus ?? "n/a"}
- Done status class: ${body.ui?.afterDone?.latestAssistantStatusClass ?? "n/a"}
- Assistant text: ${body.ui?.afterDone?.latestAssistantText ?? "n/a"}
- Citation count: ${body.ui?.afterKnowledge?.citationCount ?? "n/a"}
- Status saved into answer text: ${body.ui?.afterDone?.savedAssistantTextIncludesStatus}
...
- Status feedback only: ${body.safety?.statusFeedbackOnly}
