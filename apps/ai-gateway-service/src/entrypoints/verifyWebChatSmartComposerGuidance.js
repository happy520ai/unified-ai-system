...
try {
  ...
  await writeVerifyWebChatSmartComposerGuidanceEvidence(evidence);
...
async function writeVerifyWebChatSmartComposerGuidanceEvidence(body) {
...
function createEvidenceMarkdown(body) {
  return `# Phase 76I Web Chat Smart Composer Guidance Evidence

- Phase: ${body.phase}
- Status: ${body.status}
- Generated at: ${body.generatedAt}
- Service URL: ${body.serviceUrl ?? "n/a"}
- Hint present: ${body.ui?.initialState?.hintPresent}
- Initial guidance kind: ${body.ui?.initialState?.composerGuidanceKind ?? "n/a"}
- Typing guidance kind before probe: ${body.ui?.typedBeforeProbeState?.composerGuidanceKind ?? "n/a"}
...
- Final guidance tone: ${body.ui?.finalState?.composerGuidanceTone ?? "n/a"}
- Final guidance kind: ${body.ui?.finalState?.composerGuidanceKind ?? "n/a"}
- Send button enabled after typing: ${body.ui?.finalState?.sendDisabled === false}
