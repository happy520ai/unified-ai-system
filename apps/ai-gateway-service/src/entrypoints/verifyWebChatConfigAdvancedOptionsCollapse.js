...
try {
  ...
  await writeVerifyWebChatConfigAdvancedOptionsCollapseEvidence(evidence);
...
async function writeVerifyWebChatConfigAdvancedOptionsCollapseEvidence(body) {
...
function createEvidenceMarkdown(body) {
  return `# Phase 76G Web Chat Config Advanced Options Collapse Evidence
...
- One-click button present: ${body.ui?.initialState?.oneClickButtonPresent}
- Advanced options present: ${body.ui?.initialState?.advancedOptionsPresent}
- Advanced options open by default: ${body.ui?.initialState?.advancedOptionsOpen}
- Startup options present: ${body.ui?.initialState?.startupOptionsPresent}
- Startup options open by default: ${body.ui?.initialState?.startupOptionsOpen}
- Visible primary actions: ${(body.ui?.initialState?.visiblePrimaryActions ?? []).join(", ")}
