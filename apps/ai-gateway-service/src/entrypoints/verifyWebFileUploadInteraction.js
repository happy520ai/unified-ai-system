...
const PHASE = "phase-54a-web-file-upload-interaction";
const __dirname = dirname(fileURLToPath(import.meta.url));
const repoRoot = resolve(__dirname, "../../../..");
const evidenceDir = resolve(repoRoot, "apps/ai-gateway-service/evidence");
const evidenceJsonPath = resolve(evidenceDir, "phase-54a-web-file-upload-interaction.json");
const evidenceMdPath = resolve(evidenceDir, "phase-54a-web-file-upload-interaction.md");
const evidencePngPath = resolve(evidenceDir, "phase-54a-web-file-upload-interaction.png");

const fileName = "phase54a-upload.txt";
const marker = "phase54a-file-upload-marker";
const fileContent = `This uploaded browser file proves the Chat-first UI can load a document through the real file input and retrieve it later. ${marker}`;
const prompt = "请根据刚上传的文件说明 phase54a file upload marker 的作用。";
...
try {
  ...
  browserProfileDir = await mkdtemp(resolve(tmpdir(), "phase54a-browser-profile-"));
  ...
  try {
    ...
    await cdp.evaluate(`(() => {
      window.__phase54Fetches = [];
      window.__phase54FetchStatuses = [];
      const originalFetch = window.fetch.bind(window);
      window.fetch = async (...args) => {
        const path = String(args[0] || "");
        window.__phase54Fetches.push(path);
        const response = await originalFetch(...args);
        window.__phase54FetchStatuses.push({ path, status: response.status, ok: response.ok });
        return response;
      };
    ...
    evidence = {
      ...
      screenshot: {
        path: "apps/ai-gateway-service/evidence/phase-54a-web-file-upload-interaction.png",
      ...
      conclusion: passed ? "web-file-upload-interaction-connected" : "web-file-upload-interaction-not-connected",
    };
  } finally {
    await closeCdpSilently(cdp);
  }

  await writeVerifyWebFileUploadInteractionEvidence(evidence);
  ...
  evidence = {
    ...
    conclusion: "web-file-upload-interaction-not-connected",
  };
  await writeVerifyWebFileUploadInteractionEvidence(evidence);
  console.log(JSON.stringify(evidence, null, 2));
  process.exitCode = 1;
...
async function waitForUploadResult(cdp) {
  ...
  while (Date.now() < deadline) {
    const result = await cdp.evaluate(`(() => ({
      ...
      fetches: window.__phase54Fetches || [],
      fetchStatuses: window.__phase54FetchStatuses || [],
...
async function writeVerifyWebFileUploadInteractionEvidence(body) {
...
function createEvidenceMarkdown(body) {
  return `# Phase 54A Web File Upload Interaction Evidence
