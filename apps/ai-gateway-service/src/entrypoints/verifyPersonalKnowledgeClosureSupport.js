...
function hasUnsafePositiveStatement(text, checks) {
  const negative = /\b(no|not|never|without|blocked|disabled|forbidden|must not|does not|cannot|requires explicit|explicitly gated|preview-only|dry-run|manual|out of scope|do not|not a|not production|stop|avoid|will not|still not|is not|must remain false)\b/i;
...
export async function verifyPersonalKnowledgePhase(config) {
  const {
    phase,
    docPath,
    verifierPath,
  ...
  try {
    ...
    const checks = {
      ...
      doesNotClaimProductionRag: !hasUnsafePositiveStatement(claimScanText, [
        ["production-grade", "vector", "rag"],
        ["production", "vector", "rag"],
        ["production-grade", "graphrag"],
        ["production-ready", "rag"],
        ["production", "knowledge", "governance"],
      ]),
      doesNotClaimEnterpriseKnowledge: !hasUnsafePositiveStatement(claimScanText, [
        ["enterprise", "acl", "sync"],
        ["multi-tenant", "knowledge", "base"],
      ]),
      doesNotClaimPreviewAsProductionReady: !hasUnsafePositiveStatement(claimScanText, [
    ...
    const evidence = {
      ...
      safety: {
        ...
        enterpriseAclSyncClaimed: false,
        multiTenantKnowledgeBaseClaimed: false,
        productionKnowledgeGovernanceClaimed: false,
