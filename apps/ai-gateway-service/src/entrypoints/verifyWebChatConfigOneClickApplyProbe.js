...
const PHASE = "phase-76f-web-chat-config-one-click-apply-probe";
const __dirname = dirname(fileURLToPath(import.meta.url));
const repoRoot = resolve(__dirname, "../../../..");
const evidenceDir = resolve(repoRoot, "apps/ai-gateway-service/evidence");
const evidenceJsonPath = resolve(evidenceDir, "phase-76f-web-chat-config-one-click-apply-probe.json");
const evidenceMdPath = resolve(evidenceDir, "phase-76f-web-chat-config-one-click-apply-probe.md");
const evidencePngPath = resolve(evidenceDir, "phase-76f-web-chat-config-one-click-apply-probe.png");
...
try {
  ...
  browserProfileDir = await mkdtemp(resolve(tmpdir(), "phase76f-browser-profile-"));
  ...
  try {
    ...
    evidence = {
      ...
      screenshot: {
        path: "apps/ai-gateway-service/evidence/phase-76f-web-chat-config-one-click-apply-probe.png",
      ...
      safety: {
        ...
        oneClickApplyAndProbe: true,
        fakeProviderOnly: true,
        probeUsesExistingChatRoute: true,
        secretPersisted: false,
        backendBusinessRouteAdded: false,
        defaultChatMainLaneChanged: false,
      },
      conclusion: passed ? "web-chat-config-one-click-apply-probe-connected" : "web-chat-config-one-click-apply-probe-not-connected",
    };
  } finally {
    await closeCdpSilently(cdp);
  }

  await writeVerifyWebChatConfigOneClickApplyProbeEvidence(evidence);
  ...
  evidence = {
    ...
    conclusion: "web-chat-config-one-click-apply-probe-not-connected",
  };
  await writeVerifyWebChatConfigOneClickApplyProbeEvidence(evidence);
  console.log(JSON.stringify(evidence, null, 2));
  process.exitCode = 1;
...
async function installFetchRecorder(cdp) {
  await cdp.evaluate(`(() => {
    window.__phase76fFetches = [];
    const originalFetch = window.fetch.bind(window);
    window.fetch = async (...args) => {
      const path = String(args[0] || "");
      window.__phase76fFetches.push(path);
...
async function readState(cdp) {
  return cdp.evaluate(`(() => {
    ...
    return {
      ...
      systemMessageText: Array.from(document.querySelectorAll(".message.system .message-text")).map((node) => node.textContent || "").join("\\n"),
      ...
      fetches: window.__phase76fFetches || [],
    };
  })()`);
}
...
async function writeVerifyWebChatConfigOneClickApplyProbeEvidence(body) {
...
function createEvidenceMarkdown(body) {
  return `# Phase 76F Web Chat Config One-Click Apply Probe Evidence
...
- One-click button present: ${body.ui?.finalState?.oneClickButtonPresent}
