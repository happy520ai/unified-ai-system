...
const PHASE = "phase-76j-web-chat-top-provider-minimized";
const __dirname = dirname(fileURLToPath(import.meta.url));
const repoRoot = resolve(__dirname, "../../../..");
const evidenceDir = resolve(repoRoot, "apps/ai-gateway-service/evidence");
const evidenceJsonPath = resolve(evidenceDir, "phase-76j-web-chat-top-provider-minimized.json");
const evidenceMdPath = resolve(evidenceDir, "phase-76j-web-chat-top-provider-minimized.md");
const evidencePngPath = resolve(evidenceDir, "phase-76j-web-chat-top-provider-minimized.png");
...
try {
  ...
  browserProfileDir = await mkdtemp(resolve(tmpdir(), "phase76j-browser-profile-"));
  ...
  try {
    ...
    await waitForExpression(cdp, `document.getElementById("provider-select").value === ${JSON.stringify(selectedProvider)}`, 20_000);
    await waitForExpression(cdp, `document.getElementById("composer-model-status")?.dataset.modelProbeStatus === "passed"`, 20_000);
    const selectedState = await readState(cdp);
    ...
    const passed = !initialState.detailsPresent &&
      !initialState.summaryPresent &&
      !initialState.providerInsideDetails &&
      initialState.providerVisible === false &&
      initialState.sideButtonVisible === false &&
      openedState.wizardPresent &&
      openedState.providerVisible === false &&
      selectedState.providerValue === selectedProvider &&
      selectedState.modelProbeStatus === "passed" &&
      selectedState.fetches.includes("/chat") &&
      !selectedState.fetches.includes("/chat/rag/stream") &&
      !selectedState.fetches.includes("/chat/rag") &&
      screenshot.validPng &&
    ...
    evidence = {
      ...
      screenshot: {
        path: "apps/ai-gateway-service/evidence/phase-76j-web-chat-top-provider-minimized.png",
      ...
      safety: {
        ...
        topProviderCollapsedByDefault: true,
        providerSelectStillPresent: true,
        providerSelectionStillWorks: true,
        providerCalls: false,
        backendBusinessRouteAdded: false,
        defaultChatMainLaneChanged: false,
      },
      conclusion: passed ? "web-chat-top-provider-minimized" : "web-chat-top-provider-not-minimized",
    };
  } finally {
    await closeCdpSilently(cdp);
  }

  await writeVerifyWebChatTopProviderMinimizedEvidence(evidence);
  ...
  evidence = {
    ...
    conclusion: "web-chat-top-provider-not-minimized",
  };
  await writeVerifyWebChatTopProviderMinimizedEvidence(evidence);
  console.log(JSON.stringify(evidence, null, 2));
  process.exitCode = 1;
...
async function installFetchRecorder(cdp) {
  await cdp.evaluate(`(() => {
    window.__phase76jFetches = [];
    const originalFetch = window.fetch.bind(window);
    window.fetch = async (...args) => {
      const path = String(args[0] || "");
      window.__phase76jFetches.push(path);
...
async function readState(cdp) {
  return cdp.evaluate(`(() => {
    const details = document.getElementById("top-provider-details");
    const summary = document.getElementById("top-provider-summary");
    const select = document.getElementById("provider-select");
    const sideButton = document.getElementById("side-open");
    ...
    return {
      ...
      providerInsideDetails: Boolean(select && details && select.closest("#top-provider-details") === details),
      providerVisible: Boolean(select?.getClientRects().length && selectRect?.width > 0 && selectRect?.height > 0),
      providerValue: select?.value || "",
      ...
      sideButtonVisible: Boolean(sideButton?.getClientRects().length),
      fetches: window.__phase76jFetches || [],
    };
  })()`);
}
...
async function writeVerifyWebChatTopProviderMinimizedEvidence(body) {
...
function createEvidenceMarkdown(body) {
  return `# Phase 76J Web Chat Top Provider Minimized Evidence

- Phase: ${body.phase}
- Status: ${body.status}
- Generated at: ${body.generatedAt}
- Service URL: ${body.serviceUrl ?? "n/a"}
- Details present: ${body.ui?.initialState?.detailsPresent}
- Summary present: ${body.ui?.initialState?.summaryPresent}
- Collapsed by default: ${body.ui?.initialState?.detailsOpen === false}
- Provider select inside details: ${body.ui?.initialState?.providerInsideDetails}
- Provider select hidden by default: ${body.ui?.initialState?.providerVisible === false}
- Provider select visible when opened: ${body.ui?.openedState?.providerVisible}
- Selected provider after open: ${body.ui?.selectedState?.providerValue ?? "n/a"}
- Fetches after selection: ${JSON.stringify(body.ui?.selectedState?.fetches ?? [])}
