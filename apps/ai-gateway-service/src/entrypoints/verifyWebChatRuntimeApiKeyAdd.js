...
const PHASE = "phase-76n-web-chat-runtime-api-key-add";
const __dirname = dirname(fileURLToPath(import.meta.url));
const repoRoot = resolve(__dirname, "../../../..");
const evidenceDir = resolve(repoRoot, "apps/ai-gateway-service/evidence");
const evidenceJsonPath = resolve(evidenceDir, "phase-76n-web-chat-runtime-api-key-add.json");
const evidenceMdPath = resolve(evidenceDir, "phase-76n-web-chat-runtime-api-key-add.md");
const evidencePngPath = resolve(evidenceDir, "phase-76n-web-chat-runtime-api-key-add.png");
const selectedProvider = "local-fake-provider::local-fake-model";
const testApiKey = "fake-phase76n-secret-must-not-persist";
...
try {
  ...
  browserProfileDir = await mkdtemp(resolve(tmpdir(), "phase76n-browser-profile-"));
  ...
  try {
    ...
    await cdp.evaluate(`(() => {
      ...
      const secretInput = document.querySelector("[data-command-secret-draft]");
      secretInput.value = ${JSON.stringify(testApiKey)};
    ...
    const credentialFetch = finalState.fetches.find((item) => item.path === "/providers/runtime-credential");
    const passed = finalState.wizardPresent &&
      finalState.oneClickButtonText.includes("添加") &&
      finalState.secretPlaceholder.includes("API Key") &&
      Boolean(credentialFetch?.hasApiKey) &&
      credentialFetch?.providerId === "local-fake-provider" &&
      finalState.fetches.some((item) => item.path === "/chat") &&
    ...
    evidence = {
      ...
      screenshot: {
        path: "apps/ai-gateway-service/evidence/phase-76n-web-chat-runtime-api-key-add.png",
      ...
      safety: {
        ...
        runtimeCredentialRouteUsed: Boolean(credentialFetch),
      ...
      conclusion: passed ? "web-chat-runtime-api-key-add-connected" : "web-chat-runtime-api-key-add-not-connected",
    };
  } finally {
    await closeCdpSilently(cdp);
  }

  await writeVerifyWebChatRuntimeApiKeyAddEvidence(evidence);
  ...
  evidence = {
    ...
    conclusion: "web-chat-runtime-api-key-add-not-connected",
  };
  await writeVerifyWebChatRuntimeApiKeyAddEvidence(evidence);
  console.log(JSON.stringify(evidence, null, 2));
  process.exitCode = 1;
...
async function installFetchRecorder(cdp) {
  await cdp.evaluate(`(() => {
    window.__phase76nFetches = [];
    const originalFetch = window.fetch.bind(window);
    window.fetch = async (...args) => {
      const path = String(args[0] || "");
      const options = args[1] || {};
      const item = { path };
      if (path === "/providers/runtime-credential") {
        try {
          const parsed = JSON.parse(options.body || "{}");
          item.providerId = parsed.providerId || "";
          item.hasApiKey = Boolean(parsed.apiKey);
          item.apiKeyRedacted = parsed.apiKey ? "***" : "";
          item.apiKeyLength = parsed.apiKey ? String(parsed.apiKey).length : 0;
        } catch {
          item.bodyParseFailed = true;
        }
      }
      window.__phase76nFetches.push(item);
...
async function readState(cdp) {
  return cdp.evaluate(`(async () => {
    ...
    const provider = (providersPayload.data || []).find((item) => item.id === "local-fake-provider");
    ...
    return {
      ...
      oneClickButtonText: document.querySelector("[data-command-action='apply-and-probe-provider']")?.textContent || "",
      secretPlaceholder: secretInput?.getAttribute("placeholder") || "",
      secretInputCleared: (secretInput?.value || "") === "",
      ...
      fetches: window.__phase76nFetches || [],
    };
  })()`);
}
...
async function writeVerifyWebChatRuntimeApiKeyAddEvidence(body) {
...
function createEvidenceMarkdown(body) {
  return `# Phase 76N Web Chat Runtime API Key Add Evidence

- Phase: ${body.phase}
- Status: ${body.status}
- Generated at: ${body.generatedAt}
- Service URL: ${body.serviceUrl ?? "n/a"}
- Wizard present: ${body.ui?.finalState?.wizardPresent}
- One-click button text: ${body.ui?.finalState?.oneClickButtonText ?? "n/a"}
- Runtime credential present: ${body.ui?.finalState?.runtimeCredentialPresent}
- Credential route used: ${body.safety?.runtimeCredentialRouteUsed}
- API key value recorded: ${body.safety?.apiKeyValueRecorded}
- API key persisted in browser: ${body.safety?.apiKeyPersistedInBrowser}
- Secret storage: ${body.safety?.secretStorage}
