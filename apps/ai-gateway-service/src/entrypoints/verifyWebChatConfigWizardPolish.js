...
const evidenceJsonPath = resolve(evidenceDir, "phase-76c-web-chat-config-wizard-polish.json");
const evidenceMdPath = resolve(evidenceDir, "phase-76c-web-chat-config-wizard-polish.md");
const evidencePngPath = resolve(evidenceDir, "phase-76c-web-chat-config-wizard-polish.png");
...
try {
  ...
  browserProfileDir = await mkdtemp(resolve(tmpdir(), "phase76c-browser-profile-"));
  ...
  try {
    ...
    await cdp.evaluate(`(() => {
      ...
      document.querySelector("[data-command-action='check-secret']").click();
    ...
    const passed = initialState.wizardPresent &&
      ...
      finalState.preference?.value === selectedProvider &&
      finalState.preference?.secretPersisted === false &&
      finalState.feedbackText.includes("启动配置模板") &&
      finalState.feedbackText.includes("cmd /c pnpm dev:phase7b") &&
      finalState.feedbackText.includes("不包含你的真实 API Key") &&
      !finalState.feedbackText.includes("secret-that-must-not-persist") &&
      !finalState.localStorageDump.includes("secret-that-must-not-persist") &&
    ...
    evidence = {
      ...
      screenshot: {
        path: "apps/ai-gateway-service/evidence/phase-76c-web-chat-config-wizard-polish.png",
      ...
      conclusion: passed ? "web-chat-config-wizard-polished" : "web-chat-config-wizard-not-polished",
    };
  } finally {
...
async function installFetchRecorder(cdp) {
  await cdp.evaluate(`(() => {
    window.__phase76cFetches = [];
    const originalFetch = window.fetch.bind(window);
    window.fetch = async (...args) => {
      const path = String(args[0] || "");
      window.__phase76cFetches.push(path);
...
async function readState(cdp) {
  return cdp.evaluate(`(() => {
    ...
    return {
      ...
      userFacingTextVisible: text.includes("模型配置向导") &&
        text.includes("三步") &&
        text.includes("下一步") &&
        text.includes("不写进聊天记录") &&
        text.includes("浏览器存储") &&
      ...
      fetches: window.__phase76cFetches || [],
    };
  })()`);
}
