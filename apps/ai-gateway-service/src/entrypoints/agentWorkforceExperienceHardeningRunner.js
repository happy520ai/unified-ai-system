...
export async function executeExperienceHardeningCheck(phase, definitions, defaultGoal) {
  ...
  try {
    ...
    const previewRun = definition.needsPlan
      ? await createPreviewPlan({
        ...
        goal: definition.goal ?? defaultGoal,
      })
      : null;
    const allText = [
    ...
    await writeEvidence(phase, {
      ...
      disabledState: createDisabledState(),
      safety: createSafety(),
      conclusion: definitions[phase]?.conclusion?.replace(/complete$/, "incomplete")
        ?? "agent-workforce-experience-hardening-incomplete",
    });
    process.exitCode = 1;
  }
}
