...
const evidenceJsonPath = resolve(evidenceDir, "phase-76d-web-chat-config-effect-status.json");
const evidenceMdPath = resolve(evidenceDir, "phase-76d-web-chat-config-effect-status.md");
const evidencePngPath = resolve(evidenceDir, "phase-76d-web-chat-config-effect-status.png");
...
try {
  ...
  browserProfileDir = await mkdtemp(resolve(tmpdir(), "phase76d-browser-profile-"));
  ...
  try {
    ...
    await waitForExpression(cdp, `Boolean(document.querySelector("[data-config-effect-status='true']"))`, 20_000);
    ...
    await cdp.evaluate(`(() => {
      document.querySelector("[data-command-action='apply-provider']").click();
      return true;
    })()`);
    const appliedState = await readState(cdp);

    await cdp.evaluate(`(() => {
      document.querySelector("[data-command-action='persist-provider']").click();
      return true;
    })()`);
    const persistedState = await readState(cdp);
    ...
    const templateState = await readState(cdp);
    ...
    const passed = initialState.effectPanelPresent &&
      ...
      initialState.effectText.includes("下次打开") &&
      initialState.effectText.includes("服务重启") &&
      selectedState.effectText.includes("已选中但未应用") &&
      appliedState.effectText.includes("已生效于当前聊天") &&
      persistedState.effectText.includes("已记住") &&
      templateState.effectText.includes("启动模板已生成") &&
      templateState.effectText.includes("重启服务后才会长期生效") &&
      templateState.selectedProvider === selectedProvider &&
      templateState.preference?.value === selectedProvider &&
      !templateState.localStorageDump.includes("secret-that-must-not-persist") &&
      !templateState.fetches.includes("/chat/rag/stream") &&
      !templateState.fetches.includes("/chat/rag") &&
      screenshot.validPng &&
    ...
    evidence = {
      ...
      screenshot: {
        path: "apps/ai-gateway-service/evidence/phase-76d-web-chat-config-effect-status.png",
      ...
      conclusion: passed ? "web-chat-config-effect-status-connected" : "web-chat-config-effect-status-not-connected",
    };
  } finally {
...
async function installFetchRecorder(cdp) {
  await cdp.evaluate(`(() => {
    window.__phase76dFetches = [];
    const originalFetch = window.fetch.bind(window);
    window.fetch = async (...args) => {
      const path = String(args[0] || "");
      window.__phase76dFetches.push(path);
...
async function readState(cdp) {
  return cdp.evaluate(`(() => {
    ...
    return {
      ...
      effectItemCount: document.querySelectorAll(".chat-config-effect-item").length,
