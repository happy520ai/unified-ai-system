...
try {
  ...
  await writeVerifyWebChatCodeBlockToolsEvidence(evidence);
...
async function writeVerifyWebChatCodeBlockToolsEvidence(body) {
...
function createEvidenceMarkdown(body) {
  return `# Phase 69A Web Chat Code Block Tools Evidence
...
- Inline code count: ${body.ui?.afterAnswer?.inlineCodeCount ?? "n/a"}
- Code wrap count: ${body.ui?.afterAnswer?.codeWrapCount ?? "n/a"}
- Code block count: ${body.ui?.afterAnswer?.codeBlockCount ?? "n/a"}
- Copy code button count: ${body.ui?.afterAnswer?.copyCodeButtonCount ?? "n/a"}
- Code overflow x: ${body.ui?.afterAnswer?.codeOverflowX ?? "n/a"}
- Long code line present: ${body.ui?.afterAnswer?.longCodeLinePresent}
- Copied code matched: ${body.safety?.copyCodeButtonOnlyCopiesCode}
...
- Code block tools only: ${body.safety?.codeBlockToolsOnly}
- Inline code rendering: ${body.safety?.inlineCodeRendering}
- Long code overflow is horizontal: ${body.safety?.longCodeOverflowIsHorizontal}
