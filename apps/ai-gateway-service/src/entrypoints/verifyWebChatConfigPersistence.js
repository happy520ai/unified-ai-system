...
const PHASE = "phase-76b-web-chat-config-persistence";
const __dirname = dirname(fileURLToPath(import.meta.url));
const repoRoot = resolve(__dirname, "../../../..");
const evidenceDir = resolve(repoRoot, "apps/ai-gateway-service/evidence");
const evidenceJsonPath = resolve(evidenceDir, "phase-76b-web-chat-config-persistence.json");
const evidenceMdPath = resolve(evidenceDir, "phase-76b-web-chat-config-persistence.md");
const evidencePngPath = resolve(evidenceDir, "phase-76b-web-chat-config-persistence.png");
...
try {
  ...
  browserProfileDir = await mkdtemp(resolve(tmpdir(), "phase76b-browser-profile-"));
  ...
  try {
    ...
    await waitForExpression(cdp, `Boolean(document.querySelector("[data-command-intent='model-config']"))`);
    await cdp.evaluate(`(() => {
      const select = document.querySelector("[data-command-provider-select]");
      select.value = ${JSON.stringify(selectedProvider)};
      document.querySelector("[data-command-secret-draft]").value = "secret-that-must-not-persist";
      document.querySelector("[data-command-action='persist-provider']").click();
      document.querySelector("[data-command-action='generate-env-template']").click();
      return true;
    })()`);
    await waitForExpression(cdp, `document.getElementById("provider-select").value === ${JSON.stringify(selectedProvider)}`);
    ...
    const passed = beforeReload.selectedProvider === selectedProvider &&
      afterReload.selectedProvider === selectedProvider &&
      beforeReload.preferenceParseFailed === false &&
      afterReload.preferenceParseFailed === false &&
      beforeReload.preference?.value === selectedProvider &&
      afterReload.preference?.value === selectedProvider &&
      beforeReload.preference?.secretPersisted === false &&
      !beforeReload.localStorageDump.includes("secret-that-must-not-persist") &&
      !afterReload.localStorageDump.includes("secret-that-must-not-persist") &&
      beforeReload.commandJsonText.includes("API Key") &&
      beforeReload.commandJsonText.includes("cmd /c pnpm dev:phase7b") &&
      !beforeReload.commandJsonText.includes("secret-that-must-not-persist") &&
      !beforeReload.fetches.includes("/chat/rag/stream") &&
      !beforeReload.fetches.includes("/chat/rag") &&
      screenshot.validPng &&
    ...
    evidence = {
      ...
      screenshot: {
        path: "apps/ai-gateway-service/evidence/phase-76b-web-chat-config-persistence.png",
      ...
      safety: {
        ...
        persistedProviderModelOnly: true,
        secretPersisted: false,
        providerCalls: false,
        backendBusinessRouteAdded: false,
        defaultChatMainLaneChanged: false,
      },
      conclusion: passed ? "web-chat-config-persistence-connected" : "web-chat-config-persistence-not-connected",
    };
  } finally {
    await closeCdpSilently(cdp);
  }

  await writeVerifyWebChatConfigPersistenceEvidence(evidence);
  ...
  evidence = {
    ...
    conclusion: "web-chat-config-persistence-not-connected",
  };
  await writeVerifyWebChatConfigPersistenceEvidence(evidence);
  console.log(JSON.stringify(evidence, null, 2));
  process.exitCode = 1;
...
async function installFetchRecorder(cdp) {
  await cdp.evaluate(`(() => {
    window.__phase76bFetches = [];
    const originalFetch = window.fetch.bind(window);
    window.fetch = async (...args) => {
      const path = String(args[0] || "");
      window.__phase76bFetches.push(path);
...
async function readState(cdp) {
  return cdp.evaluate(`(() => {
    ...
    return {
      ...
      commandJsonText: document.querySelector("[data-command-json]")?.textContent || "",
      fetches: window.__phase76bFetches || [],
    };
  })()`);
}
...
async function writeVerifyWebChatConfigPersistenceEvidence(body) {
...
function createEvidenceMarkdown(body) {
  return `# Phase 76B Web Chat Config Persistence Evidence

- Phase: ${body.phase}
- Status: ${body.status}
- Generated at: ${body.generatedAt}
- Service URL: ${body.serviceUrl ?? "n/a"}
- Selected provider before reload: ${body.ui?.beforeReload?.selectedProvider ?? "n/a"}
- Selected provider after reload: ${body.ui?.afterReload?.selectedProvider ?? "n/a"}
- Preference secret persisted: ${body.ui?.afterReload?.preference?.secretPersisted}
...
- Persisted provider/model only: ${body.safety?.persistedProviderModelOnly}
