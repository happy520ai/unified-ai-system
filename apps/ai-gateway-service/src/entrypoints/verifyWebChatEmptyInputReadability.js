...
const PHASE = "phase-58a-web-chat-empty-input-readability";
const __dirname = dirname(fileURLToPath(import.meta.url));
const repoRoot = resolve(__dirname, "../../../..");
const evidenceDir = resolve(repoRoot, "apps/ai-gateway-service/evidence");
const evidenceJsonPath = resolve(evidenceDir, "phase-58a-web-chat-empty-input-readability.json");
const evidenceMdPath = resolve(evidenceDir, "phase-58a-web-chat-empty-input-readability.md");
const evidencePngPath = resolve(evidenceDir, "phase-58a-web-chat-empty-input-readability.png");

const emptyPrompt = "   \n\t   ";
const expectedHint = "请输入问题，或把文件拖进来后再发送。";
...
try {
  ...
  browserProfileDir = await mkdtemp(resolve(tmpdir(), "phase58a-browser-profile-"));
  ...
  try {
    ...
    await cdp.evaluate(`(() => {
      window.__phase58Fetches = [];
      window.__phase58FetchStatuses = [];
      const originalFetch = window.fetch.bind(window);
      window.fetch = async (...args) => {
        const path = String(args[0] || "");
        window.__phase58Fetches.push(path);
        const response = await originalFetch(...args);
        window.__phase58FetchStatuses.push({ path, status: response.status, ok: response.ok });
        return response;
      };
      return true;
    })()`);
    ...
    const passed = interaction.systemText.includes(expectedHint) &&
      interaction.inputValue === "" &&
      disallowedChatFetches.length === 0 &&
      interaction.assistantCount === 1 &&
      interaction.userCount === 0 &&
      screenshot.validPng &&
    ...
    evidence = {
      ...
      ui: {
        url: uiUrl,
        submittedWhitespaceOnly: true,
        expectedHint,
        systemText: interaction.systemText,
        hintPresent: interaction.systemText.includes(expectedHint),
        inputValueAfterSubmit: interaction.inputValue,
        ...
        assistantCount: interaction.assistantCount,
        userCount: interaction.userCount,
        systemCount: interaction.systemCount,
      },
      screenshot: {
        path: "apps/ai-gateway-service/evidence/phase-58a-web-chat-empty-input-readability.png",
      ...
      conclusion: passed ? "web-chat-empty-input-readability-connected" : "web-chat-empty-input-readability-not-connected",
    };
  } finally {
    await closeCdpSilently(cdp);
  }

  await writeVerifyWebChatEmptyInputReadabilityEvidence(evidence);
  ...
  evidence = {
    ...
    conclusion: "web-chat-empty-input-readability-not-connected",
  };
  await writeVerifyWebChatEmptyInputReadabilityEvidence(evidence);
  console.log(JSON.stringify(evidence, null, 2));
  process.exitCode = 1;
...
async function waitForEmptyInputResult(cdp) {
  ...
  while (Date.now() < deadline) {
    const result = await cdp.evaluate(`(() => {
      ...
      return {
        ...
        fetches: window.__phase58Fetches || [],
        fetchStatuses: window.__phase58FetchStatuses || [],
    ...
    if (result.systemText?.includes(expectedHint)) {
      return result;
    }
    await sleep(100);
  }
  throw new Error("Timed out waiting for browser chat empty-input readability result.");
}
...
async function writeVerifyWebChatEmptyInputReadabilityEvidence(body) {
...
function createEvidenceMarkdown(body) {
  return `# Phase 58A Web Chat Empty Input Readability Evidence

- Phase: ${body.phase}
- Status: ${body.status}
- Generated at: ${body.generatedAt}
- Service URL: ${body.serviceUrl ?? "n/a"}
- Expected hint: ${body.ui?.expectedHint ?? "n/a"}
- System text: ${body.ui?.systemText ?? "n/a"}
- Hint present: ${body.ui?.hintPresent}
- Input value after submit: ${body.ui?.inputValueAfterSubmit ?? "n/a"}
...
- Assistant count: ${body.ui?.assistantCount ?? "n/a"}
- User count: ${body.ui?.userCount ?? "n/a"}
...
- Empty input only: ${body.safety?.emptyInputOnly}
