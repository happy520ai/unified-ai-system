...
const evidenceJsonPath = resolve(evidenceDir, "phase-86a-web-chat-model-config-success-path.json");
const evidenceMdPath = resolve(evidenceDir, "phase-86a-web-chat-model-config-success-path.md");
const evidencePngPath = resolve(evidenceDir, "phase-86a-web-chat-model-config-success-path.png");
const testApiKey = "phase86-secret-must-not-persist";
const chatModelId = "phase86-chat-model";
...
try {
  ...
  browserProfileDir = await mkdtemp(resolve(tmpdir(), "phase86a-browser-profile-"));
  ...
  try {
    ...
    const evidenceDraft = {
      ...
      screenshot: {
        path: "apps/ai-gateway-service/evidence/phase-86a-web-chat-model-config-success-path.png",
      ...
      safety: {
        ...
        runtimeCredentialStoreMode: "memory",
        defaultChatMainLaneChanged: false,
      },
      conclusion: "pending",
    };

    const passed = finalState.previewFetch?.providerHint === "openai-compatible" &&
      finalState.previewFetch?.hasApiKey === true &&
      finalState.previewFetch?.baseUrl === mockProviderBaseUrl &&
      finalState.confirmFetch?.providerId === "openai-compatible" &&
      finalState.confirmFetch?.modelId === chatModelId &&
      finalState.chatFetch?.providerId === "generic-openai-compatible" &&
      finalState.chatFetch?.model === chatModelId &&
      finalState.providerSelectValue === expectedRuntimeValue &&
      finalState.commandSelectValue === expectedRuntimeValue &&
      finalState.composerModelReady === "true" &&
      finalState.composerModelProviderId === "generic-openai-compatible" &&
      finalState.composerModelId === chatModelId &&
      finalState.runtimeCredentialPresent === true &&
      finalState.preferenceValue === expectedRuntimeValue &&
      finalState.focusReturnedToChatInput === true &&
      finalState.secretInputCleared === true &&
    ...
    evidence = {
      ...
      conclusion: passed ? "web-chat-model-config-success-path-connected" : "web-chat-model-config-success-path-not-connected",
    };
  } finally {
        ...
        data: [
          {
            id: chatModelId,
            object: "model",
            owned_by: "phase86-mock",
            capabilities: ["chat", "summary"],
            input_modalities: ["text"],
            output_modalities: ["text"],
          },
          {
            id: "phase86-vision-model",
            object: "model",
            owned_by: "phase86-mock",
            capabilities: ["vision"],
    ...
    if (request.method === "POST" && request.url === "/v1/chat/completions") {
      return sendJson(response, 200, {
        ...
        choices: [{
          ...
          message: {
            role: "assistant",
            content: "phase86 success path answer",
          },
          finish_reason: "stop",
        }],
        usage: {
    ...
    source: `
      (() => {
        ...
        window.fetch = async (...args) => {
          ...
          window.__phase86Fetches.push(item);
...
async function readState(cdp) {
  return cdp.evaluate(`(async () => {
    const fetches = window.__phase86Fetches || [];
    const providersPayload = await fetch("/providers").then((response) => response.json());
    const provider = (providersPayload.data || []).find((item) => item.id === "generic-openai-compatible");
    ...
    return {
      ...
      commandSelectValue: document.querySelector("[data-command-provider-select]")?.value || "",
      composerModelStatus: document.getElementById("composer-model-status")?.textContent || "",
      composerModelStatusState: document.getElementById("composer-model-status")?.dataset.modelProbeStatus || "",
      composerModelReady: document.getElementById("composer-model-status")?.dataset.modelReady || "",
      composerModelProviderId: document.getElementById("composer-model-status")?.dataset.modelProviderId || "",
      composerModelId: document.getElementById("composer-model-status")?.dataset.modelId || "",
      composerModelGuideText: document.getElementById("composer-model-guide")?.textContent || "",
      feedbackText: document.querySelector("[data-command-feedback]")?.textContent || "",
      runtimeCredentialPresent: Boolean(provider?.metadata?.runtimeCredentialPresent),
      runtimeModelIds: (provider?.models || []).map((model) => model.id),
      preferenceValue: preference.value || "",
      preferenceProviderId: preference.providerId || "",
      preferenceModelId: preference.modelId || "",
      ...
      previewFetch: fetches.find((item) => item.path === "/models/import/preview") || null,
      confirmFetch: fetches.find((item) => item.path === "/models/import/confirm") || null,
      chatFetch: fetches.find((item) => item.path === "/chat") || null,
      fetches,
    };
  })()`);
}
