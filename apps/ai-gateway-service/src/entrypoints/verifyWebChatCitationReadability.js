...
const PHASE = "phase-66a-web-chat-citation-readability";
const __dirname = dirname(fileURLToPath(import.meta.url));
const repoRoot = resolve(__dirname, "../../../..");
const evidenceDir = resolve(repoRoot, "apps/ai-gateway-service/evidence");
const evidenceJsonPath = resolve(evidenceDir, "phase-66a-web-chat-citation-readability.json");
const evidenceMdPath = resolve(evidenceDir, "phase-66a-web-chat-citation-readability.md");
const evidencePngPath = resolve(evidenceDir, "phase-66a-web-chat-citation-readability.png");

const prompt = "phase66a citation readability prompt";
const answer = "phase66a answer with readable citations";
const firstSnippet = "phase66a first citation snippet";
const secondSnippet = "phase66a second citation snippet";
...
try {
  ...
  browserProfileDir = await mkdtemp(resolve(tmpdir(), "phase66a-browser-profile-"));
  ...
  try {
    ...
    await waitForExpression(cdp, `(() => {
      const state = window.__phase66ReadState();
      return state.latestAssistantText.includes(${JSON.stringify(answer)}) &&
        state.citationsVisible &&
        state.citationCount === 2 &&
        state.firstCitationOpen === true &&
        state.copySingleButtonCount === 2;
    })()`);
    const afterAnswer = await readState(cdp);

    await cdp.evaluate(`(() => {
      const summaries = window.__phase66LatestAssistant().querySelectorAll(".citation-card summary");
      summaries[1].click();
      return true;
    })()`);
    await waitForExpression(cdp, "window.__phase66ReadState().secondCitationOpen === true");
    const afterExpandSecond = await readState(cdp);

    await cdp.evaluate(`(() => {
      const buttons = window.__phase66LatestAssistant().querySelectorAll('[data-message-action="copy-single-citation"]');
      buttons[0].click();
      return true;
    })()`);
    await waitForExpression(cdp, `window.__phase66ReadState().clipboard.some((item) => item.includes(${JSON.stringify(firstSnippet)}))`);
    const afterCopySingle = await readState(cdp);
    ...
    const passed = afterAnswer.latestAssistantText.includes(answer) &&
      afterAnswer.citationHeading.includes("引用 2 条") &&
      afterAnswer.citationCount === 2 &&
      afterAnswer.firstCitationOpen === true &&
      afterExpandSecond.secondCitationOpen === true &&
      afterCopySingle.clipboard.some((item) => item.includes(firstSnippet)) &&
      !afterCopySingle.clipboard.at(-1)?.includes(secondSnippet) &&
      screenshot.validPng &&
    ...
    evidence = {
      ...
      screenshot: {
        path: "apps/ai-gateway-service/evidence/phase-66a-web-chat-citation-readability.png",
      ...
      conclusion: passed ? "web-chat-citation-readability-connected" : "web-chat-citation-readability-not-connected",
    };
  } finally {
    await closeCdpSilently(cdp);
  }

  await writeVerifyWebChatCitationReadabilityEvidence(evidence);
  ...
  evidence = {
    ...
    conclusion: "web-chat-citation-readability-not-connected",
  };
  await writeVerifyWebChatCitationReadabilityEvidence(evidence);
  console.log(JSON.stringify(evidence, null, 2));
  process.exitCode = 1;
...
async function installChatSimulation(cdp) {
  await cdp.evaluate(`(() => {
    window.__phase66Fetches = [];
    window.__phase66FetchStatuses = [];
    window.__phase66Clipboard = [];
    Object.defineProperty(navigator, "clipboard", {
      configurable: true,
      value: {
        writeText: async (text) => {
          window.__phase66Clipboard.push(String(text));
        },
      },
    });
    window.__phase66LatestAssistant = () => {
      const messages = Array.from(document.querySelectorAll(".message.assistant"));
      return messages[messages.length - 1];
    };
    window.__phase66ReadState = () => {
      const latestAssistant = window.__phase66LatestAssistant();
      ...
      return {
        ...
        citationsVisible: Boolean(latestAssistant?.querySelector(".message-citations:not(.hidden)")),
        citationCount: citationCards.length,
        firstCitationOpen: Boolean(citationCards[0]?.open),
        secondCitationOpen: Boolean(citationCards[1]?.open),
        firstSummary: citationCards[0]?.querySelector("summary")?.textContent || "",
        secondSummary: citationCards[1]?.querySelector("summary")?.textContent || "",
        firstSnippet: citationCards[0]?.querySelector(".citation-snippet")?.textContent || "",
        secondSnippet: citationCards[1]?.querySelector(".citation-snippet")?.textContent || "",
        copySingleButtonCount: latestAssistant?.querySelectorAll('[data-message-action="copy-single-citation"]').length || 0,
        clipboard: window.__phase66Clipboard || [],
        fetches: window.__phase66Fetches || [],
        fetchStatuses: window.__phase66FetchStatuses || [],
    ...
    window.fetch = async (...args) => {
      ...
      window.__phase66Fetches.push(path);
      if (path === "/chat/rag/stream") {
        const citations = [
          {
            sourceId: "phase66a-source",
            documentId: "phase66a-document-1",
            title: "Phase66A first document",
            snippet: ${JSON.stringify(firstSnippet)},
          },
          {
            sourceId: "phase66a-source",
            documentId: "phase66a-document-2",
            title: "Phase66A second document",
            snippet: ${JSON.stringify(secondSnippet)},
          },
        ];
        ...
        window.__phase66FetchStatuses.push({ path, status: response.status, ok: response.ok, simulated: true });
        return response;
      }
      const response = await originalFetch(...args);
      window.__phase66FetchStatuses.push({ path, status: response.status, ok: response.ok, simulated: false });
      return response;
    };
    return true;
  })()`);
...
async function readState(cdp) {
  return cdp.evaluate("window.__phase66ReadState()");
}
...
async function waitForExpression(cdp, expression, timeoutMs = 15_000) {
  ...
  let debugState = null;
  try {
    debugState = await cdp.evaluate(`typeof window.__phase66ReadState === "function" ? window.__phase66ReadState() : null`);
  } catch (error) {
...
async function writeVerifyWebChatCitationReadabilityEvidence(body) {
...
function createEvidenceMarkdown(body) {
  return `# Phase 66A Web Chat Citation Readability Evidence
...
- Citation count: ${body.ui?.afterAnswer?.citationCount ?? "n/a"}
- First citation open: ${body.ui?.afterAnswer?.firstCitationOpen}
- Second citation open after click: ${body.ui?.afterExpandSecond?.secondCitationOpen}
- First summary: ${body.ui?.afterAnswer?.firstSummary ?? "n/a"}
- Second summary: ${body.ui?.afterAnswer?.secondSummary ?? "n/a"}
- Copied single citation: ${(body.ui?.afterCopySingle?.clipboard || []).join(" | ")}
...
- Citation readability only: ${body.safety?.citationReadabilityOnly}
