...
const tenantAdmin = { userId: "tenant-admin", tenantId: "t1", role: "admin" };
...
describe("enterprise-governance-extended", () => {
  it("creates user and retrieves by token", () => {
    ...
    service.upsertUser({ userId: "test-1", tenantId: "t1", role: "operator", token: "tok-123" }, tenantAdmin);
    const users = service.listUsers(tenantAdmin);
    expect(users.users.some((u) => u.userId === "test-1")).toBe(true);
    service.revokeUser({ userId: "test-1" }, tenantAdmin);
  });

  it("exports users for backup without exposing tokens", () => {
    const service = createEnterpriseGovernanceService({ env: {} });
    service.upsertUser({ userId: "test-2", tenantId: "t1", role: "viewer", token: "tok-456" }, tenantAdmin);
    const backup = service.exportUsersForBackup(tenantAdmin);
    expect(backup.tokenValuesExposed).toBe(false);
    expect(backup.configuredUsers.some((u) => u.userId === "test-2")).toBe(true);
    service.revokeUser({ userId: "test-2" }, tenantAdmin);
  });
  ...
  it("records and exports audit as JSONL", async () => {
    ...
    const exported = await service.exportAudit({ format: "jsonl", limit: 10, actorIdentity: tenantAdmin });
