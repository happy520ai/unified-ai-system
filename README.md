...
| Read-only operator console (opt-in) | `AI_GATEWAY_CONSOLE_ENABLED=true` serves a self-contained, build-free dashboard page at `GET /console` (overview snapshot, virtual keys, local clients, cache audit) behind the same authentication and `dashboard:read` authorization as every other route. Default off keeps the gateway terminal-first (`/console` stays 404); no mutation surface. | [Observability](docs/observability-export.md) |
