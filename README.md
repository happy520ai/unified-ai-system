...
| Multi-instance ready | `AI_GATEWAY_MULTI_INSTANCE=true` switches rate limiting, idempotency, and A2A tasks to same-host shared SQLite. PostgreSQL modes cover cross-host quotas/idempotency/WebSocket leases, fenced Workforce ownership, central billable usage, and a shared HMAC audit chain; complete distributed queue/HA, WORM retention, and provider-invoice reconciliation remain explicit. | [Multi-process deployment](docs/multi-process-deployment.md) |
