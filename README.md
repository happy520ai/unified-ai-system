...
| Operator traffic control | Configurable **weighted routing splits** and **shadow traffic** (`AI_GATEWAY_WEIGHTED_ROUTES_JSON`): split requests across providers by weight; shadow calls observe and log without double-billing. | [Multi-process deployment](docs/multi-process-deployment.md) |
| Hot-path RAG + billing ledger | Opt-in `unified_ai.rag` knowledge injection on `/v1/chat/completions`; a local JSONL billing ledger (customers, usage, invoices, voids, manual payments — clearly labeled as statements, not legal invoices, no payment gateway). | [Spend reporting](docs/spend-reporting.md) |
| Multi-instance ready | `AI_GATEWAY_MULTI_INSTANCE=true` switches rate limiting and idempotency to same-host shared SQLite stores with a shared HMAC secret; postgres modes remain available for cross-host. | [Multi-process deployment](docs/multi-process-deployment.md) |
