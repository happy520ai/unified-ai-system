...
| Multi-instance ready | `AI_GATEWAY_MULTI_INSTANCE=true` switches rate limiting, idempotency, and A2A tasks to same-host shared SQLite. PostgreSQL modes cover cross-host quotas/idempotency/WebSocket leases and fenced Workforce ownership; complete distributed queue/HA evidence remains explicit. | [Multi-process deployment](docs/multi-process-deployment.md) |
