...
| Connect another agent | [A2A v1.0 gateway](docs/a2a-protocol.md) | Verify an optional signed Agent Card/JWKS and execute tenant-scoped fake-provider tasks, with bounded memory, same-host SQLite, or cross-host PostgreSQL storage. |
...
| Multi-instance ready | `AI_GATEWAY_MULTI_INSTANCE=true` keeps same-host SQLite defaults. Explicit PostgreSQL modes cover cross-host quotas/idempotency/WebSocket leases, A2A task lifecycle, fenced Workforce ownership, central billable usage, and a shared HMAC audit chain; complete distributed execution/HA, external WORM, and authenticated provider-statement ingestion remain explicit. | [Multi-process deployment](docs/multi-process-deployment.md) |
