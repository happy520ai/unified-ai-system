...
import { phase599AllowedBlockers, phase599Group, phase599SafetyBoundary, phase599Subphases } from "./phase599-registry.mjs";
...
for (const phase of phase599Subphases) {
  ...
  const phasePassed =
    ...
    phase599AllowedBlockers.includes(evidence.blocker) &&
    ...
    evidence.realCodexConnectionMade === false &&
    evidence.relayStarted === false &&
    evidence.realConfigWriteAllowed === false &&
    evidence.relayStartAllowed === false &&
    evidence.realIntegrationAllowed === false &&
    evidence.guardedRealTestAllowed === false &&
...
report.completed = report.phases.length === phase599Group.subphases.length && report.phases.every((item) => item.completed === true);
...
async function readClosureAggregate() {
  try {
    return JSON.parse(await readFile(resolve(repoRoot, phase599Group.sequenceEvidencePath), "utf8"));
  } catch {
    return null;
  }
}
...
async function writeTopReport(payload = report) {
  await writeFileWithRetry(resolve(repoRoot, phase599Group.sequenceEvidencePath), `${JSON.stringify(payload, null, 2)}\n`);
}
