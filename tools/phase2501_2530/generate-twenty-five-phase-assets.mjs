...
async function buildProposalDiff(target, beforeText, afterText) {
  ...
  const tempDir = resolve("tmp/phase2501-2530-proposals");
...
function buildDoc() {
  return `# Phase2501A-2530A Controlled Twenty-Five Tool Mutation
  ...
  return JSON.stringify(
    {
      ...
      planId: "phase2501-2530-controlled-twenty-five-tool-mutation",
      maxChangedFiles: 25,
      runNodeCheck: true,
      runLocalSmoke: true,
      approvalRecord: {
        ...
        approvedAtLeastOnceByOwner: [
          {
            ...
            scope: "phase2501-2530-controlled-twenty-five-tool-mutation",
          },
        ],
...
${smokeMarkers}
  ...
  const result = {
    ...
    blocker: completed ? "none" : "twenty_five_mutation_node_check_or_smoke_failed",
...
function buildBaseResult({ plan, validation, generatedAt, planPath }) {
  return {
    ...
    phase2500Sealed: validation.upstreamResults.some((entry) => entry.id === "${previousPhaseMeta.sealCheckId}" && entry.passed),
    codexExecExecuted: false,
    providerCallsMade: false,
    projectProviderCallsMade: false,
    secretRead: false,
...
function printSummary(result) {
  console.log(JSON.stringify({
    status: result.status,
    blocker: result.blocker,
    twentyFiveMutationApplied: result.twentyFiveMutationApplied,
    changedFileCount: result.changedFileCount,
    nodeCheckPassed: result.nodeCheckPassed,
    localTwentyFiveSmokePassed: result.localTwentyFiveSmokePassed,
