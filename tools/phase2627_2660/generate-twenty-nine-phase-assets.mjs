...
async function buildProposalDiff(target, beforeText, afterText) {
  ...
  const tempDir = resolve("tmp/phase2627-2660-proposals");
...
function buildDoc() {
  return `# Phase2627A-2660A Controlled Twenty-Nine Tool Mutation
  ...
  return JSON.stringify(
    {
      ...
      planId: "phase2627-2660-controlled-twenty-nine-tool-mutation",
      maxChangedFiles: 29,
      runNodeCheck: true,
      runLocalSmoke: true,
      approvalRecord: {
        ...
        approvedAtLeastOnceByOwner: [
          {
            ...
            scope: "phase2627-2660-controlled-twenty-nine-tool-mutation",
          },
        ],
...
function buildBaseResult({ plan, validation, generatedAt, planPath }) {
  return {
    ...
    phase2626Sealed: validation.upstreamResults.some((entry) => entry.id === "${previousPhaseMeta.sealCheckId}" && entry.passed),
    codexExecExecuted: false,
    providerCallsMade: false,
    projectProviderCallsMade: false,
    secretRead: false,
...
const phase2626 = readJson("${previousPhaseMeta.resultPath}") || {};
...
check("${previousPhaseMeta.sealCheckId}", phase2626.recommendedSealed === true && phase2626.${previousPhaseMeta.sealCheckField} === true);
