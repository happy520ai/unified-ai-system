...
import { phase602AllowedBlockers, phase602Group, phase602SafetyBoundary, phase602Subphases } from "./phase602-registry.mjs";

const repoRoot = resolve(fileURLToPath(new URL("..", import.meta.url)));
const commandTimeoutMs = Number(process.env.PHASE602_COMMAND_TIMEOUT_MS || 90 * 60 * 1000);
const report = { phaseRange: "Phase602A-T", title: phase602Group.title, completed: false, recommended_sealed: false, blocker: null, phases: [], safetyBoundary: { ...phase602SafetyBoundary } };
...
for (const phase of phase602Subphases) {
  ...
  const phasePassed =
    ...
    phase602AllowedBlockers.includes(evidence.blocker) &&
    evidence.requestAttemptCount <= 1 &&
    evidence.retryAttemptCount === 0 &&
    evidence.sessionOverrideUsed === true &&
    evidence.persistentConfigWriteDetected === false &&
    evidence.userCodexConfigModified === false &&
    evidence.projectCodexConfigModified === false &&
    evidence.realConfigWritePerformed === false &&
...
report.completed = report.phases.length === phase602Group.subphases.length && report.phases.every((item) => item.completed === true);
...
async function readClosureAggregate() {
  try {
    return JSON.parse(await readFile(resolve(repoRoot, phase602Group.sequenceEvidencePath), "utf8"));
  } catch {
    return null;
  }
}
...
async function writeTopReport(payload = report) {
  await writeFile(resolve(repoRoot, phase602Group.sequenceEvidencePath), `${JSON.stringify(payload, null, 2)}\n`);
}
