...
async function buildProposalDiff(target, beforeText, afterText) {
  ...
  const tempDir = resolve("tmp/phase2417-2443-proposals");
...
function buildDoc() {
  return `# Phase2417A-2443A Controlled Twenty-Two Tool Mutation
  ...
  return JSON.stringify(
    {
      ...
      planId: "phase2417-2443-controlled-twenty-two-tool-mutation",
      maxChangedFiles: 22,
      runNodeCheck: true,
      runLocalSmoke: true,
      approvalRecord: {
        ...
        approvedAtLeastOnceByOwner: [
          {
            ...
            scope: "phase2417-2443-controlled-twenty-two-tool-mutation",
          },
        ],
...
function buildBaseResult({ plan, validation, generatedAt, planPath }) {
  return {
    ...
    phase2416Sealed: validation.upstreamResults.some((entry) => entry.id === "${previousPhaseMeta.sealCheckId}" && entry.passed),
    codexExecExecuted: false,
    providerCallsMade: false,
    projectProviderCallsMade: false,
    secretRead: false,
...
const phase2416 = readJson("${previousPhaseMeta.resultPath}") || {};
...
check("${previousPhaseMeta.sealCheckId}", phase2416.recommendedSealed === true && phase2416.${previousPhaseMeta.sealCheckField} === true);
