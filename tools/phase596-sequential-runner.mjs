...
import { phase596Group, phase596SafetyBoundary, phase596Subphases } from "./phase596-registry.mjs";
...
for (const phase of phase596Subphases) {
...
report.completed = report.phases.length === phase596Group.subphases.length && report.phases.every((item) => item.completed === true);
...
async function readClosureAggregate() {
  try {
    return JSON.parse(await readFile(resolve(repoRoot, phase596Group.sequenceEvidencePath), "utf8"));
  } catch {
    return null;
  }
}
...
async function writeTopReport(payload = report) {
  await writeFileWithRetry(resolve(repoRoot, phase596Group.sequenceEvidencePath), `${JSON.stringify(payload, null, 2)}\n`);
}
