...
async function buildProposalDiff(target, beforeText, afterText) {
  ...
  const tempDir = resolve("tmp/phase3194-3241-proposals");
...
function buildDoc() {
  return `# Phase3194A-3241A Controlled Forty-Three Tool Mutation
  ...
  return JSON.stringify(
    {
      ...
      planId: "phase3194-3241-controlled-forty-three-tool-mutation",
      maxChangedFiles: 43,
      runNodeCheck: true,
      runLocalSmoke: true,
      approvalRecord: {
        ...
        approvedAtLeastOnceByOwner: [
          {
            ...
            scope: "phase3194-3241-controlled-forty-three-tool-mutation",
          },
        ],
...
function buildBaseResult({ plan, validation, generatedAt, planPath }) {
  return {
    ...
    phase3193Sealed: validation.upstreamResults.some((entry) => entry.id === "${previousPhaseMeta.sealCheckId}" && entry.passed),
    codexExecExecuted: false,
    providerCallsMade: false,
    projectProviderCallsMade: false,
    secretRead: false,
...
const phase3193 = readJson("${previousPhaseMeta.resultPath}") || {};
...
check("${previousPhaseMeta.sealCheckId}", phase3193.recommendedSealed === true && phase3193.${previousPhaseMeta.sealCheckField} === true);
