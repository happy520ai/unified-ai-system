...
async function buildProposalDiff(target, beforeText, afterText) {
  ...
  const tempDir = resolve("tmp/phase3777-3835-proposals");
...
function buildDoc() {
  return `# Phase3777A-3835A Controlled Fifty-Four Tool Mutation
  ...
  return JSON.stringify(
    {
      ...
      planId: "phase3777-3835-controlled-fifty-four-tool-mutation",
      maxChangedFiles: 54,
      runNodeCheck: true,
      runLocalSmoke: true,
      approvalRecord: {
        ...
        approvedAtLeastOnceByOwner: [
          {
            ...
            scope: "phase3777-3835-controlled-fifty-four-tool-mutation",
          },
        ],
...
function buildBaseResult({ plan, validation, generatedAt, planPath }) {
  return {
    ...
    phase3776Sealed: validation.upstreamResults.some((entry) => entry.id === "${previousPhaseMeta.sealCheckId}" && entry.passed),
    codexExecExecuted: false,
    providerCallsMade: false,
    projectProviderCallsMade: false,
    secretRead: false,
...
function buildVerifierTemplate() {
  ...
  const packageSmoke = "node tools/phase3777_3835/smoke-controlled-fifty-four-tool-mutation.mjs";
...
const phase3776 = readJson("${previousPhaseMeta.resultPath}") || {};
...
check("${previousPhaseMeta.sealCheckId}", phase3776.recommendedSealed === true && phase3776.${previousPhaseMeta.sealCheckField} === true);
...
async function updatePackageScripts() {
  ...
  const smokeScript = "node tools/phase3777_3835/smoke-controlled-fifty-four-tool-mutation.mjs";
