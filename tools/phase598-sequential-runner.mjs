...
import { phase598Group, phase598SafetyBoundary, phase598Subphases } from "./phase598-registry.mjs";
...
for (const phase of phase598Subphases) {
...
report.completed = report.phases.length === phase598Group.subphases.length && report.phases.every((item) => item.completed === true);
...
async function readClosureAggregate() {
  try {
    return JSON.parse(await readFile(resolve(repoRoot, phase598Group.sequenceEvidencePath), "utf8"));
  } catch {
    return null;
  }
}
...
async function writeTopReport(payload = report) {
  await writeFileWithRetry(resolve(repoRoot, phase598Group.sequenceEvidencePath), `${JSON.stringify(payload, null, 2)}\n`);
}
