...
async function buildProposalDiff(target, beforeText, afterText) {
  ...
  const tempDir = resolve("tmp/phase3242-3290-proposals");
...
function buildDoc() {
  return `# Phase3242A-3290A Controlled Forty-Four Tool Mutation
  ...
  return JSON.stringify(
    {
      ...
      planId: "phase3242-3290-controlled-forty-four-tool-mutation",
      maxChangedFiles: 44,
      runNodeCheck: true,
      runLocalSmoke: true,
      approvalRecord: {
        ...
        approvedAtLeastOnceByOwner: [
          {
            ...
            scope: "phase3242-3290-controlled-forty-four-tool-mutation",
          },
        ],
...
${smokeMarkers}
  ...
  const result = {
    ...
    blocker: completed ? "none" : "forty_four_mutation_node_check_or_smoke_failed",
...
function validatePlan(plan) {
  return validateControlledMutationPlan({
    ...
    expectedOperationCount: 44,
    expectedMaxChangedFiles: 44,
...
function buildBaseResult({ plan, validation, generatedAt, planPath }) {
  return {
    ...
    phase3241Sealed: validation.upstreamResults.some((entry) => entry.id === "${previousPhaseMeta.sealCheckId}" && entry.passed),
    codexExecExecuted: false,
    providerCallsMade: false,
    projectProviderCallsMade: false,
    secretRead: false,
...
function printSummary(result) {
  console.log(JSON.stringify({
    status: result.status,
    blocker: result.blocker,
    fortyFourMutationApplied: result.fortyFourMutationApplied,
    changedFileCount: result.changedFileCount,
    nodeCheckPassed: result.nodeCheckPassed,
    localFortyFourSmokePassed: result.localFortyFourSmokePassed,
...
const phase3241 = readJson("${previousPhaseMeta.resultPath}") || {};
...
check("${previousPhaseMeta.sealCheckId}", phase3241.recommendedSealed === true && phase3241.${previousPhaseMeta.sealCheckField} === true);
