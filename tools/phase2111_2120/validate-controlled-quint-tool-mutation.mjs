...
const runnerPath = "tools/phase2111_2120/apply-controlled-quint-tool-mutation.mjs";
const substratePath = "tools/phase2101_2110/controlled-mutation-substrate.mjs";
const docsPath = "docs/phase2111-2120-controlled-quint-tool-mutation.md";
const approvalPath = "docs/phase2111-2120-controlled-quint-tool-mutation-approval.example.json";
const proposalOnePath = "docs/phase2116-quint-tool-mutation-target-one.proposal.diff";
const proposalTwoPath = "docs/phase2117-quint-tool-mutation-target-two.proposal.diff";
const proposalThreePath = "docs/phase2118-quint-tool-mutation-target-three.proposal.diff";
const proposalFourPath = "docs/phase2119-quint-tool-mutation-target-four.proposal.diff";
const proposalFivePath = "docs/phase2120-quint-tool-mutation-target-five.proposal.diff";
...
const evidenceDir = "apps/ai-gateway-service/evidence/phase2111-2120-controlled-quint-tool-mutation";
...
const smokePath = `${evidenceDir}/quint-smoke.json`;
const checks = [];
...
const phase2110 = readJson("apps/ai-gateway-service/evidence/phase2101-2110-controlled-quad-tool-mutation/result.json") || {};
...
check(
  ...
  packageJson.scripts?.["apply:phase2111-2120-controlled-quint-tool-mutation"] ===
    "node tools/phase2111_2120/apply-controlled-quint-tool-mutation.mjs --plan docs/phase2111-2120-controlled-quint-tool-mutation-approval.example.json",
);
check(
  "package_smoke_script_registered",
  packageJson.scripts?.["smoke:phase2111-2120-controlled-quint-tool-mutation"] ===
...
check(
  ...
  packageJson.scripts?.["verify:phase2111-2120-controlled-quint-tool-mutation"] ===
    "node tools/phase2111_2120/validate-controlled-quint-tool-mutation.mjs",
);
check("phase632_preflight_passed", phase632.preflightPassed === true && phase632.staleFalse === true);
check("phase2110_sealed", phase2110.recommendedSealed === true && phase2110.quadMutationApplied === true);
check("result_exists", result !== null, "run apply:phase2111-2120-controlled-quint-tool-mutation first");
...
if (result) {
  ...
  check("quint_mutation_applied", result.quintMutationApplied === true);
  check("changed_file_count_five", result.changedFileCount === 5);
  check(
  ...
  check("local_smokes_passed", result.localQuintSmokePassed === true);
...
if (rollback) {
  ...
  check("rollback_restore_quint", rollback.rollbackAction === "restore-previous-content-quint");
  check("rollback_not_executed", rollback.rollbackExecuted === false);
  check("rollback_five_entries", Array.isArray(rollback.files) && rollback.files.length === 5);
}
...
if (smoke) {
  ...
  check("smoke_phase2116_marker", smoke.phase2116Marker === "PHASE2116_QUINT_TOOL_TARGET_ONE_OK");
  check("smoke_phase2094_marker", smoke.phase2094Marker === "PHASE2094_BATCH_TOOL_TARGET_TWO_OK");
  check("smoke_phase2117_marker", smoke.phase2117Marker === "PHASE2117_QUINT_TOOL_TARGET_TWO_OK");
  check("smoke_phase2100_marker", smoke.phase2100Marker === "PHASE2100_TRIPLE_TOOL_TARGET_THREE_OK");
  check("smoke_phase2118_marker", smoke.phase2118Marker === "PHASE2118_QUINT_TOOL_TARGET_THREE_OK");
  check("smoke_phase2109_marker", smoke.phase2109Marker === "PHASE2109_QUAD_TOOL_TARGET_FOUR_OK");
  check("smoke_phase2119_marker", smoke.phase2119Marker === "PHASE2119_QUINT_TOOL_TARGET_FOUR_OK");
...
check("target_one_phase2116_export", targetOne.includes("export function buildPhase2116QuintMutationTargetOneStatus"));
check("target_one_phase2116_marker", targetOne.includes("PHASE2116_QUINT_TOOL_TARGET_ONE_OK"));
check("target_two_phase2117_export", targetTwo.includes("export function buildPhase2117QuintMutationTargetTwoStatus"));
check("target_two_phase2117_marker", targetTwo.includes("PHASE2117_QUINT_TOOL_TARGET_TWO_OK"));
check("target_three_phase2118_export", targetThree.includes("export function buildPhase2118QuintMutationTargetThreeStatus"));
check("target_three_phase2118_marker", targetThree.includes("PHASE2118_QUINT_TOOL_TARGET_THREE_OK"));
check("target_four_phase2119_export", targetFour.includes("export function buildPhase2119QuintMutationTargetFourStatus"));
check("target_four_phase2119_marker", targetFour.includes("PHASE2119_QUINT_TOOL_TARGET_FOUR_OK"));
check("target_five_phase2120_export", targetFive.includes("export function buildPhase2120QuintMutationRuntimeStatus"));
check("target_five_phase2120_marker", targetFive.includes("PHASE2120_QUINT_TOOL_TARGET_FIVE_OK"));
...
check("targets_no_plain_secret_value", !hasPlainSecretValue(targetOne) && !hasPlainSecretValue(targetTwo) && !hasPlainSecretValue(targetThree) && !hasPlainSecretValue(targetFour) && !hasPlainSecretValue(targetFive));
check("docs_mentions_quint", docs.includes("controlled quint tool mutation"));
...
const verifierResult = {
  ...
  phase2110Sealed: phase2110.recommendedSealed === true,
  quintMutationReady: completed,
  changedFiles: [targetOnePath, targetTwoPath, targetThreePath, targetFourPath, targetFivePath],
  quintMutationApplied: result?.quintMutationApplied === true,
  nodeCheckPassed: result?.nodeCheckPassed === true,
  localQuintSmokePassed: result?.localQuintSmokePassed === true,
...
console.log(JSON.stringify({
  ...
  quintMutationApplied: verifierResult.quintMutationApplied,
  nodeCheckPassed: verifierResult.nodeCheckPassed,
  localQuintSmokePassed: verifierResult.localQuintSmokePassed,
}, null, 2));
