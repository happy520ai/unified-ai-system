...
import { phase600AllowedBlockers, phase600Group, phase600SafetyBoundary, phase600Subphases } from "./phase600-registry.mjs";
...
for (const phase of phase600Subphases) {
  ...
  const phasePassed =
    ...
    phase600AllowedBlockers.includes(evidence.blocker) &&
...
report.completed = report.phases.length === phase600Group.subphases.length && report.phases.every((item) => item.completed === true);
...
async function readClosureAggregate() {
  try {
    return JSON.parse(await readFile(resolve(repoRoot, phase600Group.sequenceEvidencePath), "utf8"));
  } catch {
    return null;
  }
}
...
async function writeTopReport(payload = report) {
  await writeFile(resolve(repoRoot, phase600Group.sequenceEvidencePath), `${JSON.stringify(payload, null, 2)}\n`);
}
