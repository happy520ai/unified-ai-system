...
const DEFAULT_SERIAL_REPORT_PATH = resolve(
  process.cwd(),
  ".tmp",
  "client-runtime-mainstream-serial-summary.json",
);
...
function getPassedArgs(rawArgs) {
  ...
  for (let index = 0; index < rawArgs.length; index += 1) {
    ...
    if (arg.startsWith("--")) {
      if (arg === "--client-catalog" || arg === "--client-catalog-dir" || arg === "--client-catalog-url") {
...
const evidenceArg = getArgValueRaw(rawArgs, "evidence");
...
const main = () => {
  if (!skipTemplate) {
    if (forceTemplate || !existsSync(evidencePath)) {
      ...
      console.log(`Using existing mainstream evidence file: ${evidencePath}`);
    }
  }

  console.log(`Running mainstream clients strictly with evidence: ${evidencePath}`);
...
try {
  ...
  process.stderr.write(`Mainstream client certification failed: ${error?.message ?? error}\n`);
  process.exit(1);
}
