...
async function buildProposalDiff(target, beforeText, afterText) {
  ...
  const tempDir = resolve("tmp/phase3551-3605-proposals");
...
function buildDoc() {
  return `# Phase3551A-3605A Controlled Fifty Tool Mutation
  ...
  return JSON.stringify(
    {
      ...
      planId: "phase3551-3605-controlled-fifty-tool-mutation",
      maxChangedFiles: 50,
      runNodeCheck: true,
      runLocalSmoke: true,
      approvalRecord: {
        ...
        approvedAtLeastOnceByOwner: [
          {
            ...
            scope: "phase3551-3605-controlled-fifty-tool-mutation",
          },
        ],
...
function buildBaseResult({ plan, validation, generatedAt, planPath }) {
  return {
    ...
    phase3550Sealed: validation.upstreamResults.some((entry) => entry.id === "${previousPhaseMeta.sealCheckId}" && entry.passed),
    codexExecExecuted: false,
    providerCallsMade: false,
    projectProviderCallsMade: false,
    secretRead: false,
...
function buildVerifierTemplate() {
  ...
  const packageSmoke = "node tools/phase3551_3605/smoke-controlled-fifty-tool-mutation.mjs";
...
const phase3550 = readJson("${previousPhaseMeta.resultPath}") || {};
...
check("${previousPhaseMeta.sealCheckId}", phase3550.recommendedSealed === true && phase3550.${previousPhaseMeta.sealCheckField} === true);
...
async function updatePackageScripts() {
  ...
  const smokeScript = "node tools/phase3551_3605/smoke-controlled-fifty-tool-mutation.mjs";
