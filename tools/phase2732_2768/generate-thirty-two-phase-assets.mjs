...
async function buildProposalDiff(target, beforeText, afterText) {
  ...
  const tempDir = resolve("tmp/phase2732-2768-proposals");
...
function buildDoc() {
  return `# Phase2732A-2768A Controlled Thirty-Two Tool Mutation
  ...
  return JSON.stringify(
    {
      ...
      planId: "phase2732-2768-controlled-thirty-two-tool-mutation",
      maxChangedFiles: 32,
      runNodeCheck: true,
      runLocalSmoke: true,
      approvalRecord: {
        ...
        approvedAtLeastOnceByOwner: [
          {
            ...
            scope: "phase2732-2768-controlled-thirty-two-tool-mutation",
          },
        ],
...
${smokeMarkers}
  ...
  const result = {
    ...
    blocker: completed ? "none" : "thirty_two_mutation_node_check_or_smoke_failed",
...
function validatePlan(plan) {
  return validateControlledMutationPlan({
    ...
    expectedOperationCount: 32,
    expectedMaxChangedFiles: 32,
...
function buildBaseResult({ plan, validation, generatedAt, planPath }) {
  return {
    ...
    phase2731Sealed: validation.upstreamResults.some((entry) => entry.id === "${previousPhaseMeta.sealCheckId}" && entry.passed),
    codexExecExecuted: false,
    providerCallsMade: false,
    projectProviderCallsMade: false,
    secretRead: false,
...
function printSummary(result) {
  console.log(JSON.stringify({
    status: result.status,
    blocker: result.blocker,
    thirtyTwoMutationApplied: result.thirtyTwoMutationApplied,
    changedFileCount: result.changedFileCount,
    nodeCheckPassed: result.nodeCheckPassed,
    localThirtyTwoSmokePassed: result.localThirtyTwoSmokePassed,
...
const phase2731 = readJson("${previousPhaseMeta.resultPath}") || {};
...
check("${previousPhaseMeta.sealCheckId}", phase2731.recommendedSealed === true && phase2731.${previousPhaseMeta.sealCheckField} === true);
