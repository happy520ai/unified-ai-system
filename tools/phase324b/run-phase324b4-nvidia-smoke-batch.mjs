...
const phase = "Phase324B-4";
const batchId = "phase324b4";
...
const evidenceDir = path.join(repoRoot, "apps", "ai-gateway-service", "evidence", "phase324b4");
const batchResultJsonPath = path.join(docsDir, "phase324b4-nvidia-smoke-batch-result.json");
const batchReportMdPath = path.join(docsDir, "phase324b4-nvidia-smoke-batch-report.md");
const evidenceIndexJsonPath = path.join(docsDir, "phase324b4-model-smoke-evidence-index.json");
...
async function buildPreviousProcessedSet() {
...
async function buildCandidatePreview() {
  const [inventory, phase324eReport, phase324eDashboard, phase324d2fStrategy, previousProcessed] = await Promise.all([
    readJson(inventoryPath),
    readJson(phase324eReportPath),
    readJson(phase324eDashboardPath),
    readJson(phase324d2fStrategyPath),
    buildPreviousProcessedSet(),
  ]);
  ...
  for (const record of matrix.records ?? []) {
    ...
    else if (highRiskModels.has(record.modelId)) reason = "known_failed_or_high_risk";
    else if (previousProcessed.has(record.modelId)) reason = "previously_processed";
  ...
  const preview = {
    ...
    excludedHighRiskModels: excluded.filter((item) => item.reason === "known_failed_or_high_risk").map((item) => item.modelId),
...
function classifyResult({ result, missingKey }) {
  ...
  if (
    ...
    return {
      ...
      notes: ["Real NVIDIA call completed with non-empty assistant text; Phase324C-3 may review selectable eligibility."],
    };
  }
...
function renderBatchMarkdown(summary) {
  const lines = [
    ...
    `- eligibleForPhase324C3Count: ${summary.eligibleForPhase324C3Count}`,
    ...
    "",
    `- eligible_for_phase324c3_selectable_review: ${summary.phase324c3Candidates.length ? summary.phase324c3Candidates.map((item) => `\`${item}\``).join(", ") : "none"}`,
    `- not_eligible_for_phase324c3: ${summary.nonPhase324c3Candidates.length ? summary.nonPhase324c3Candidates.map((item) => `\`${item}\``).join(", ") : "none"}`,
    "",
    "## Boundary",
    "",
...
function buildSummary({ preview, results }) {
  const phase324c3Candidates = results
    .filter((item) => item.finalStatus === "smoke_passed" && item.selectableRecommendation === "eligible_after_phase324c3")
    .map((item) => item.modelId);
  const nonPhase324c3Candidates = results
    .filter((item) => !phase324c3Candidates.includes(item.modelId))
    .map((item) => item.modelId);
  ...
  return {
    ...
    eligibleForPhase324C3Count: phase324c3Candidates.length,
    ...
    results,
    phase324c3Candidates,
    nonPhase324c3Candidates,
