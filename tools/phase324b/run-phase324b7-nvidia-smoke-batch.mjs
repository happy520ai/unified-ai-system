...
const phase = "Phase324B-7";
const batchId = "phase324b7";
const providerId = "nvidia";
const promptText = "请用一句中文回答：你现在能稳定完成普通聊天任务吗？";
...
const evidenceDir = path.join(repoRoot, "apps", "ai-gateway-service", "evidence", "phase324b7");
const resultPath = path.join(docsDir, "phase324b7-nvidia-smoke-batch-result.json");
const reportPath = path.join(docsDir, "phase324b7-nvidia-smoke-batch-report.md");
const indexPath = path.join(docsDir, "phase324b7-model-smoke-evidence-index.json");
const executionReportPath = path.join(docsDir, "phase324b7-execution-report.md");
...
const batchResultPaths = [
  ...
  "phase324b6-nvidia-smoke-batch-result.json",
...
const evidenceIndexPaths = [
  ...
  "phase324b6-model-smoke-evidence-index.json",
].map((name) => path.join(docsDir, name));
...
async function buildPriorSets() {
  ...
  for (const item of existingIndex?.evidence ?? []) {
    processed.add(item.modelId);
  }
...
function classify({ result, missingKey }) {
  ...
  if (providerCalled && httpStatus === 200 && completionVerified && assistantTextPresent && !errorCode) {
    return { finalStatus: "smoke_passed", selectableRecommendation: "eligible_after_phase324c6" };
  }
...
function renderReport(summary) {
  const lines = [
    ...
    `- eligibleForPhase324C6Count: ${summary.summary.eligibleForPhase324C6Count}`,
    "",
    "## Results",
    "",
...
function renderExecutionReport(summary) {
  const lines = [
    "# Phase324B-7 Execution Report",
    "",
    `- previewExecuted: true`,
    `- runExecuted: ${summary.mode === "run"}`,
    `- plannedCount: ${summary.preview.plannedSmokeCount}`,
    `- processedCount: ${summary.summary.processedCount}`,
    `- smokePassedCount: ${summary.summary.smokePassedCount}`,
    `- smokeFailedCount: ${summary.summary.smokeFailedCount}`,
    `- skippedEnvMissingCount: ${summary.summary.skippedEnvMissingCount}`,
    ...
    `- verificationStateModified: false`,
    "",
  ];
  return lines.join("\n");
}
...
async function run() {
  ...
  if (preview.plannedSmokeCount === 0) {
    ...
    await writeFile(executionReportPath, `${renderExecutionReport(skipped)}\n`, "utf8");
    return skipped;
  }
  ...
  const summary = {
    ...
    summary: {
      ...
      eligibleForPhase324C6Count: results.filter((item) => item.selectableRecommendation === "eligible_after_phase324c6").length,
    },
    results,
    preview,
    safety: {
