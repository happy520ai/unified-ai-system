...
async function buildProposalDiff(target, beforeText, afterText) {
  ...
  const tempDir = resolve("tmp/phase3662-3718-proposals");
...
function buildDoc() {
  return `# Phase3662A-3718A Controlled Fifty-Two Tool Mutation
  ...
  return JSON.stringify(
    {
      ...
      planId: "phase3662-3718-controlled-fifty-two-tool-mutation",
      maxChangedFiles: 52,
      runNodeCheck: true,
      runLocalSmoke: true,
      approvalRecord: {
        ...
        approvedAtLeastOnceByOwner: [
          {
            ...
            scope: "phase3662-3718-controlled-fifty-two-tool-mutation",
          },
        ],
...
function buildBaseResult({ plan, validation, generatedAt, planPath }) {
  return {
    ...
    phase3661Sealed: validation.upstreamResults.some((entry) => entry.id === "${previousPhaseMeta.sealCheckId}" && entry.passed),
    codexExecExecuted: false,
    providerCallsMade: false,
    projectProviderCallsMade: false,
    secretRead: false,
...
function buildVerifierTemplate() {
  ...
  const packageSmoke = "node tools/phase3662_3718/smoke-controlled-fifty-two-tool-mutation.mjs";
...
const phase3661 = readJson("${previousPhaseMeta.resultPath}") || {};
...
check("${previousPhaseMeta.sealCheckId}", phase3661.recommendedSealed === true && phase3661.${previousPhaseMeta.sealCheckField} === true);
...
async function updatePackageScripts() {
  ...
  const smokeScript = "node tools/phase3662_3718/smoke-controlled-fifty-two-tool-mutation.mjs";
