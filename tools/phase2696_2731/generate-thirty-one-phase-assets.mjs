...
async function buildProposalDiff(target, beforeText, afterText) {
  ...
  const tempDir = resolve("tmp/phase2696-2731-proposals");
...
function buildDoc() {
  return `# Phase2696A-2731A Controlled Thirty-One Tool Mutation
  ...
  return JSON.stringify(
    {
      ...
      planId: "phase2696-2731-controlled-thirty-one-tool-mutation",
      maxChangedFiles: 31,
      runNodeCheck: true,
      runLocalSmoke: true,
      approvalRecord: {
        ...
        approvedAtLeastOnceByOwner: [
          {
            ...
            scope: "phase2696-2731-controlled-thirty-one-tool-mutation",
          },
        ],
...
function buildBaseResult({ plan, validation, generatedAt, planPath }) {
  return {
    ...
    phase2695Sealed: validation.upstreamResults.some((entry) => entry.id === "${previousPhaseMeta.sealCheckId}" && entry.passed),
    codexExecExecuted: false,
    providerCallsMade: false,
    projectProviderCallsMade: false,
    secretRead: false,
...
const phase2695 = readJson("${previousPhaseMeta.resultPath}") || {};
...
check("${previousPhaseMeta.sealCheckId}", phase2695.recommendedSealed === true && phase2695.${previousPhaseMeta.sealCheckField} === true);
