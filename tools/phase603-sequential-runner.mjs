...
import { phase603AllowedBlockers, phase603Group, phase603SafetyBoundary, phase603Subphases } from "./phase603-registry.mjs";

const repoRoot = resolve(fileURLToPath(new URL("..", import.meta.url)));
const commandTimeoutMs = Number(process.env.PHASE603_COMMAND_TIMEOUT_MS || 90 * 60 * 1000);
const report = { phaseRange: "Phase603A-T", title: phase603Group.title, completed: false, recommended_sealed: false, blocker: null, phases: [], safetyBoundary: { ...phase603SafetyBoundary } };

for (const phase of phase603Subphases) {
  ...
  const phasePassed =
    ...
    phase603AllowedBlockers.includes(evidence.blocker) &&
    evidence.designAndPreparationOnly === true &&
    ...
    evidence.realProviderSwitchExecuted === false &&
    evidence.providerCallsMade === false &&
    evidence.realTestExecuted === false &&
    evidence.commandExecuted === false &&
...
report.completed = report.phases.length === phase603Group.subphases.length && report.phases.every((item) => item.completed === true);
...
async function readClosureAggregate() {
  try {
    return JSON.parse(await readFile(resolve(repoRoot, phase603Group.sequenceEvidencePath), "utf8"));
  } catch {
    return null;
  }
}
...
async function writeTopReport(payload = report) {
  await writeFile(resolve(repoRoot, phase603Group.sequenceEvidencePath), `${JSON.stringify(payload, null, 2)}\n`);
}
