...
import { phase601AllowedBlockers, phase601Group, phase601SafetyBoundary, phase601Subphases } from "./phase601-registry.mjs";

const repoRoot = resolve(fileURLToPath(new URL("..", import.meta.url)));
const commandTimeoutMs = Number(process.env.PHASE601_COMMAND_TIMEOUT_MS || 90 * 60 * 1000);
const report = {
  phaseRange: "Phase601A-T",
  title: phase601Group.title,
  completed: false,
...
for (const phase of phase601Subphases) {
  ...
  const phasePassed =
    ...
    phase601AllowedBlockers.includes(evidence.blocker) &&
...
report.completed = report.phases.length === phase601Group.subphases.length && report.phases.every((item) => item.completed === true);
...
async function readClosureAggregate() {
  try {
    return JSON.parse(await readFile(resolve(repoRoot, phase601Group.sequenceEvidencePath), "utf8"));
  } catch {
    return null;
  }
}
...
async function writeTopReport(payload = report) {
  await writeFile(resolve(repoRoot, phase601Group.sequenceEvidencePath), `${JSON.stringify(payload, null, 2)}\n`);
}
