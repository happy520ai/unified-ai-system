...
async function buildProposalDiff(target, beforeText, afterText) {
  ...
  const tempDir = resolve("tmp/phase3392-3443-proposals");
...
function buildDoc() {
  return `# Phase3392A-3443A Controlled Forty-Seven Tool Mutation
  ...
  return JSON.stringify(
    {
      ...
      planId: "phase3392-3443-controlled-forty-seven-tool-mutation",
      maxChangedFiles: 47,
      runNodeCheck: true,
      runLocalSmoke: true,
      approvalRecord: {
        ...
        approvedAtLeastOnceByOwner: [
          {
            ...
            scope: "phase3392-3443-controlled-forty-seven-tool-mutation",
          },
        ],
...
function buildBaseResult({ plan, validation, generatedAt, planPath }) {
  return {
    ...
    phase3391Sealed: validation.upstreamResults.some((entry) => entry.id === "${previousPhaseMeta.sealCheckId}" && entry.passed),
    codexExecExecuted: false,
    providerCallsMade: false,
    projectProviderCallsMade: false,
    secretRead: false,
...
function buildVerifierTemplate() {
  ...
  const packageSmoke = "node tools/phase3392_3443/smoke-controlled-forty-seven-tool-mutation.mjs";
...
const phase3391 = readJson("${previousPhaseMeta.resultPath}") || {};
...
check("${previousPhaseMeta.sealCheckId}", phase3391.recommendedSealed === true && phase3391.${previousPhaseMeta.sealCheckField} === true);
...
async function updatePackageScripts() {
  ...
  const smokeScript = "node tools/phase3392_3443/smoke-controlled-forty-seven-tool-mutation.mjs";
