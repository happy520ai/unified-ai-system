...
async function buildProposalDiff(target, beforeText, afterText) {
  ...
  const tempDir = resolve("tmp/phase2391-2416-proposals");
...
function buildDoc() {
  return `# Phase2391A-2416A Controlled Twenty-One Tool Mutation
  ...
  return JSON.stringify(
    {
      ...
      planId: "phase2391-2416-controlled-twenty-one-tool-mutation",
      maxChangedFiles: 21,
      runNodeCheck: true,
      runLocalSmoke: true,
      approvalRecord: {
        ...
        approvedAtLeastOnceByOwner: [
          {
            ...
            scope: "phase2391-2416-controlled-twenty-one-tool-mutation",
          },
        ],
...
function buildBaseResult({ plan, validation, generatedAt, planPath }) {
  return {
    ...
    phase2390Sealed: validation.upstreamResults.some((entry) => entry.id === "${previousPhaseMeta.sealCheckId}" && entry.passed),
    codexExecExecuted: false,
    providerCallsMade: false,
    projectProviderCallsMade: false,
    secretRead: false,
...
const phase2390 = readJson("${previousPhaseMeta.resultPath}") || {};
...
check("${previousPhaseMeta.sealCheckId}", phase2390.recommendedSealed === true && phase2390.${previousPhaseMeta.sealCheckField} === true);
