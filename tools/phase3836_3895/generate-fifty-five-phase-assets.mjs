...
async function buildProposalDiff(target, beforeText, afterText) {
  ...
  const tempDir = resolve("tmp/phase3836-3895-proposals");
...
function buildDoc() {
  return `# Phase3836A-3895A Controlled Fifty-Five Tool Mutation
  ...
  return JSON.stringify(
    {
      ...
      planId: "phase3836-3895-controlled-fifty-five-tool-mutation",
      maxChangedFiles: 55,
      runNodeCheck: true,
      runLocalSmoke: true,
      approvalRecord: {
        ...
        approvedAtLeastOnceByOwner: [
          {
            ...
            scope: "phase3836-3895-controlled-fifty-five-tool-mutation",
          },
        ],
...
function buildBaseResult({ plan, validation, generatedAt, planPath }) {
  return {
    ...
    phase3835Sealed: validation.upstreamResults.some((entry) => entry.id === "${previousPhaseMeta.sealCheckId}" && entry.passed),
    codexExecExecuted: false,
    providerCallsMade: false,
    projectProviderCallsMade: false,
    secretRead: false,
...
function buildVerifierTemplate() {
  ...
  const packageSmoke = "node tools/phase3836_3895/smoke-controlled-fifty-five-tool-mutation.mjs";
...
const phase3835 = readJson("${previousPhaseMeta.resultPath}") || {};
...
check("${previousPhaseMeta.sealCheckId}", phase3835.recommendedSealed === true && phase3835.${previousPhaseMeta.sealCheckField} === true);
...
async function updatePackageScripts() {
  ...
  const smokeScript = "node tools/phase3836_3895/smoke-controlled-fifty-five-tool-mutation.mjs";
