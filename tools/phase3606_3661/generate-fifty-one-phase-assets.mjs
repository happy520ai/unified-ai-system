...
async function buildProposalDiff(target, beforeText, afterText) {
  ...
  const tempDir = resolve("tmp/phase3606-3661-proposals");
...
function buildDoc() {
  return `# Phase3606A-3661A Controlled Fifty-One Tool Mutation
  ...
  return JSON.stringify(
    {
      ...
      planId: "phase3606-3661-controlled-fifty-one-tool-mutation",
      maxChangedFiles: 51,
      runNodeCheck: true,
      runLocalSmoke: true,
      approvalRecord: {
        ...
        approvedAtLeastOnceByOwner: [
          {
            ...
            scope: "phase3606-3661-controlled-fifty-one-tool-mutation",
          },
        ],
...
function buildBaseResult({ plan, validation, generatedAt, planPath }) {
  return {
    ...
    phase3605Sealed: validation.upstreamResults.some((entry) => entry.id === "${previousPhaseMeta.sealCheckId}" && entry.passed),
    codexExecExecuted: false,
    providerCallsMade: false,
    projectProviderCallsMade: false,
    secretRead: false,
...
function buildVerifierTemplate() {
  ...
  const packageSmoke = "node tools/phase3606_3661/smoke-controlled-fifty-one-tool-mutation.mjs";
...
const phase3605 = readJson("${previousPhaseMeta.resultPath}") || {};
...
check("${previousPhaseMeta.sealCheckId}", phase3605.recommendedSealed === true && phase3605.${previousPhaseMeta.sealCheckField} === true);
...
async function updatePackageScripts() {
  ...
  const smokeScript = "node tools/phase3606_3661/smoke-controlled-fifty-one-tool-mutation.mjs";
