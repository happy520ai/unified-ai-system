...
function mutateTargetContent(beforeText, target) {
  ...
  if (target.addMode === "runtime-plus-export") {
    let next = beforeText;
    const anchorMap = {
      2303: "export function buildPhase2243FourteenMutationTargetTwoStatus() {",
      2304: "export function buildPhase2244FourteenMutationTargetThreeStatus() {",
      2305: "export function buildPhase2245FourteenMutationTargetFourStatus() {",
      2306: "export function buildPhase2246FourteenMutationTargetFiveStatus() {",
    };
...
async function buildProposalDiff(target, beforeText, afterText) {
  ...
  const tempDir = resolve("tmp/phase2297-2318-proposals");
...
function buildDoc() {
  return `# Phase2297A-2318A Controlled Seventeen Tool Mutation
  ...
  return JSON.stringify(
    {
      ...
      planId: "phase2297-2318-controlled-seventeen-tool-mutation",
      maxChangedFiles: 17,
      runNodeCheck: true,
      runLocalSmoke: true,
      approvalRecord: {
        ...
        approvedAtLeastOnceByOwner: [
          {
            ...
            scope: "phase2297-2318-controlled-seventeen-tool-mutation",
          },
        ],
...
function buildBaseResult({ plan, validation, generatedAt, planPath }) {
  return {
    ...
    phase2296Sealed: validation.upstreamResults.some((entry) => entry.id === "${previousPhaseMeta.sealCheckId}" && entry.passed),
    codexExecExecuted: false,
    providerCallsMade: false,
    projectProviderCallsMade: false,
    secretRead: false,
...
const phase2296 = readJson("${previousPhaseMeta.resultPath}") || {};
...
check("${previousPhaseMeta.sealCheckId}", phase2296.recommendedSealed === true && phase2296.${previousPhaseMeta.sealCheckField} === true);
