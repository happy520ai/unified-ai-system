...
async function buildProposalDiff(target, beforeText, afterText) {
  ...
  const tempDir = resolve("tmp/phase3291-3340-proposals");
...
function buildDoc() {
  return `# Phase3291A-3340A Controlled Forty-Five Tool Mutation
  ...
  return JSON.stringify(
    {
      ...
      planId: "phase3291-3340-controlled-forty-five-tool-mutation",
      maxChangedFiles: 45,
      runNodeCheck: true,
      runLocalSmoke: true,
      approvalRecord: {
        ...
        approvedAtLeastOnceByOwner: [
          {
            ...
            scope: "phase3291-3340-controlled-forty-five-tool-mutation",
          },
        ],
...
function buildBaseResult({ plan, validation, generatedAt, planPath }) {
  return {
    ...
    phase3290Sealed: validation.upstreamResults.some((entry) => entry.id === "${previousPhaseMeta.sealCheckId}" && entry.passed),
    codexExecExecuted: false,
    providerCallsMade: false,
    projectProviderCallsMade: false,
    secretRead: false,
...
const phase3290 = readJson("${previousPhaseMeta.resultPath}") || {};
...
check("${previousPhaseMeta.sealCheckId}", phase3290.recommendedSealed === true && phase3290.${previousPhaseMeta.sealCheckField} === true);
