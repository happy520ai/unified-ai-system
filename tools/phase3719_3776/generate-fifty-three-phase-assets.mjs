...
async function buildProposalDiff(target, beforeText, afterText) {
  ...
  const tempDir = resolve("tmp/phase3719-3776-proposals");
...
function buildDoc() {
  return `# Phase3719A-3776A Controlled Fifty-Three Tool Mutation
  ...
  return JSON.stringify(
    {
      ...
      planId: "phase3719-3776-controlled-fifty-three-tool-mutation",
      maxChangedFiles: 53,
      runNodeCheck: true,
      runLocalSmoke: true,
      approvalRecord: {
        ...
        approvedAtLeastOnceByOwner: [
          {
            ...
            scope: "phase3719-3776-controlled-fifty-three-tool-mutation",
          },
        ],
...
function buildBaseResult({ plan, validation, generatedAt, planPath }) {
  return {
    ...
    phase3718Sealed: validation.upstreamResults.some((entry) => entry.id === "${previousPhaseMeta.sealCheckId}" && entry.passed),
    codexExecExecuted: false,
    providerCallsMade: false,
    projectProviderCallsMade: false,
    secretRead: false,
...
function buildVerifierTemplate() {
  ...
  const packageSmoke = "node tools/phase3719_3776/smoke-controlled-fifty-three-tool-mutation.mjs";
...
const phase3718 = readJson("${previousPhaseMeta.resultPath}") || {};
...
check("${previousPhaseMeta.sealCheckId}", phase3718.recommendedSealed === true && phase3718.${previousPhaseMeta.sealCheckField} === true);
...
async function updatePackageScripts() {
  ...
  const smokeScript = "node tools/phase3719_3776/smoke-controlled-fifty-three-tool-mutation.mjs";
