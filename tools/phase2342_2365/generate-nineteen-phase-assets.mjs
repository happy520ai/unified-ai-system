...
async function buildProposalDiff(target, beforeText, afterText) {
  ...
  const tempDir = resolve("tmp/phase2342-2365-proposals");
...
function buildDoc() {
  return `# Phase2342A-2365A Controlled Nineteen Tool Mutation
  ...
  return JSON.stringify(
    {
      ...
      planId: "phase2342-2365-controlled-nineteen-tool-mutation",
      maxChangedFiles: 19,
      runNodeCheck: true,
      runLocalSmoke: true,
      approvalRecord: {
        ...
        approvedAtLeastOnceByOwner: [
          {
            ...
            scope: "phase2342-2365-controlled-nineteen-tool-mutation",
          },
        ],
...
function buildBaseResult({ plan, validation, generatedAt, planPath }) {
  return {
    ...
    phase2341Sealed: validation.upstreamResults.some((entry) => entry.id === "${previousPhaseMeta.sealCheckId}" && entry.passed),
    codexExecExecuted: false,
    providerCallsMade: false,
    projectProviderCallsMade: false,
    secretRead: false,
...
const phase2341 = readJson("${previousPhaseMeta.resultPath}") || {};
...
check("${previousPhaseMeta.sealCheckId}", phase2341.recommendedSealed === true && phase2341.${previousPhaseMeta.sealCheckField} === true);
