...
async function buildProposalDiff(target, beforeText, afterText) {
  ...
  const tempDir = resolve("tmp/phase2366-2390-proposals");
...
function buildDoc() {
  return `# Phase2366A-2390A Controlled Twenty Tool Mutation
  ...
  return JSON.stringify(
    {
      ...
      planId: "phase2366-2390-controlled-twenty-tool-mutation",
      maxChangedFiles: 20,
      runNodeCheck: true,
      runLocalSmoke: true,
      approvalRecord: {
        ...
        approvedAtLeastOnceByOwner: [
          {
            ...
            scope: "phase2366-2390-controlled-twenty-tool-mutation",
          },
        ],
...
function buildBaseResult({ plan, validation, generatedAt, planPath }) {
  return {
    ...
    phase2365Sealed: validation.upstreamResults.some((entry) => entry.id === "${previousPhaseMeta.sealCheckId}" && entry.passed),
    codexExecExecuted: false,
    providerCallsMade: false,
    projectProviderCallsMade: false,
    secretRead: false,
...
const phase2365 = readJson("${previousPhaseMeta.resultPath}") || {};
...
check("${previousPhaseMeta.sealCheckId}", phase2365.recommendedSealed === true && phase2365.${previousPhaseMeta.sealCheckField} === true);
