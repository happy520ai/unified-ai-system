...
const reviewPath = path.join(repoRoot, "docs", "phase324c2-selectable-model-review.json");
const verificationStatePath = path.join(repoRoot, "apps", "ai-gateway-service", "evidence", "phase-313a-model-verification-state.json");
const updateReportPath = path.join(repoRoot, "docs", "phase324c2-model-registry-update-report.md");
...
function buildSmokeResultFromEvidence(evidence) {
  return {
    success: true,
    code: "nvidia_call_ok",
    message: "Phase324C-2 adopted Phase324B-2/B-3 reviewed smoke evidence.",
...
function buildUpdatedRecord(existingRecord, evidence) {
  return {
    ...
    lastSmokeMode: "phase324c2_review_adopted",
...
function renderReport({ plans, rejectedModelIds, targetFile }) {
  const lines = [
    "# Phase324C-2 Model Registry Update Report",
    "",
...
async function main() {
  ...
  assert(review.evidenceMissing !== true, "Phase324C-2 review found missing evidence.");
  assert(review.evidenceModelMismatch !== true, "Phase324C-2 review found evidence/modelId mismatch.");
  assert((review.eligibleModels ?? []).length === 5, "Phase324C-2 review eligible model count is not 5.");
  assert((review.rejectedModels ?? []).length === 5, "Phase324C-2 review rejected model count is not 5.");
  ...
  assert(plans.length === 5, "Dry-run/apply plan contains more than 5 models.");
