...
async function buildProposalDiff(target, beforeText, afterText) {
  ...
  const tempDir = resolve("tmp/phase3101-3146-proposals");
...
function buildDoc() {
  return `# Phase3101A-3146A Controlled Forty-One Tool Mutation
  ...
  return JSON.stringify(
    {
      ...
      planId: "phase3101-3146-controlled-forty-one-tool-mutation",
      maxChangedFiles: 41,
      runNodeCheck: true,
      runLocalSmoke: true,
      approvalRecord: {
        ...
        approvedAtLeastOnceByOwner: [
          {
            ...
            scope: "phase3101-3146-controlled-forty-one-tool-mutation",
          },
        ],
...
${smokeMarkers}
  ...
  const result = {
    ...
    blocker: completed ? "none" : "forty_one_mutation_node_check_or_smoke_failed",
...
function validatePlan(plan) {
  return validateControlledMutationPlan({
    ...
    expectedOperationCount: 41,
    expectedMaxChangedFiles: 41,
...
function buildBaseResult({ plan, validation, generatedAt, planPath }) {
  return {
    ...
    phase3100Sealed: validation.upstreamResults.some((entry) => entry.id === "${previousPhaseMeta.sealCheckId}" && entry.passed),
    codexExecExecuted: false,
    providerCallsMade: false,
    projectProviderCallsMade: false,
    secretRead: false,
...
function printSummary(result) {
  console.log(JSON.stringify({
    status: result.status,
    blocker: result.blocker,
    fortyOneMutationApplied: result.fortyOneMutationApplied,
    changedFileCount: result.changedFileCount,
    nodeCheckPassed: result.nodeCheckPassed,
    localFortyOneSmokePassed: result.localFortyOneSmokePassed,
...
const phase3100 = readJson("${previousPhaseMeta.resultPath}") || {};
...
check("${previousPhaseMeta.sealCheckId}", phase3100.recommendedSealed === true && phase3100.${previousPhaseMeta.sealCheckField} === true);
