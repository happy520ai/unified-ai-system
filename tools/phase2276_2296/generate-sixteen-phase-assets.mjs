...
function mutateTargetContent(beforeText, target) {
  ...
  if (target.addMode === "runtime-plus-export") {
    let next = beforeText;
    const anchorMap = {
      2282: "export function buildPhase2243FourteenMutationTargetTwoStatus() {",
      2283: "export function buildPhase2244FourteenMutationTargetThreeStatus() {",
      2284: "export function buildPhase2245FourteenMutationTargetFourStatus() {",
      2285: "export function buildPhase2246FourteenMutationTargetFiveStatus() {",
    };
...
async function buildProposalDiff(target, beforeText, afterText) {
  ...
  const tempDir = resolve("tmp/phase2276-2296-proposals");
...
function buildDoc() {
  return `# Phase2276A-2296A Controlled Sixteen Tool Mutation
  ...
  return JSON.stringify(
    {
      ...
      planId: "phase2276-2296-controlled-sixteen-tool-mutation",
      maxChangedFiles: 16,
      runNodeCheck: true,
      runLocalSmoke: true,
      approvalRecord: {
        ...
        approvedAtLeastOnceByOwner: [
          {
            ...
            scope: "phase2276-2296-controlled-sixteen-tool-mutation",
          },
        ],
...
${buildStatusFunction(targets[15]).trim()}
...
function buildBaseResult({ plan, validation, generatedAt, planPath }) {
  return {
    ...
    phase2275Sealed: validation.upstreamResults.some((entry) => entry.id === "${previousPhaseMeta.sealCheckId}" && entry.passed),
    codexExecExecuted: false,
    providerCallsMade: false,
    projectProviderCallsMade: false,
    secretRead: false,
...
const phase2275 = readJson("${previousPhaseMeta.resultPath}") || {};
...
check("${previousPhaseMeta.sealCheckId}", phase2275.recommendedSealed === true && phase2275.${previousPhaseMeta.sealCheckField} === true);
