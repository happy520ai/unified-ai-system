...
async function buildProposalDiff(target, beforeText, afterText) {
  ...
  const tempDir = resolve("tmp/phase2769-2806-proposals");
...
function buildDoc() {
  return `# Phase2769A-2806A Controlled Thirty-Three Tool Mutation
  ...
  return JSON.stringify(
    {
      ...
      planId: "phase2769-2806-controlled-thirty-three-tool-mutation",
      maxChangedFiles: 33,
      runNodeCheck: true,
      runLocalSmoke: true,
      approvalRecord: {
        ...
        approvedAtLeastOnceByOwner: [
          {
            ...
            scope: "phase2769-2806-controlled-thirty-three-tool-mutation",
          },
        ],
...
${smokeMarkers}
  ...
  const result = {
    ...
    blocker: completed ? "none" : "thirty_three_mutation_node_check_or_smoke_failed",
...
function buildBaseResult({ plan, validation, generatedAt, planPath }) {
  return {
    ...
    phase2768Sealed: validation.upstreamResults.some((entry) => entry.id === "${previousPhaseMeta.sealCheckId}" && entry.passed),
    codexExecExecuted: false,
    providerCallsMade: false,
    projectProviderCallsMade: false,
    secretRead: false,
...
function printSummary(result) {
  console.log(JSON.stringify({
    status: result.status,
    blocker: result.blocker,
    thirtyThreeMutationApplied: result.thirtyThreeMutationApplied,
    changedFileCount: result.changedFileCount,
    nodeCheckPassed: result.nodeCheckPassed,
    localThirtyThreeSmokePassed: result.localThirtyThreeSmokePassed,
...
const phase2768 = readJson("${previousPhaseMeta.resultPath}") || {};
...
check("${previousPhaseMeta.sealCheckId}", phase2768.recommendedSealed === true && phase2768.${previousPhaseMeta.sealCheckField} === true);
