...
import { phase597Group, phase597SafetyBoundary, phase597Subphases } from "./phase597-registry.mjs";
...
for (const phase of phase597Subphases) {
...
report.completed = report.phases.length === phase597Group.subphases.length && report.phases.every((item) => item.completed === true);
...
async function readClosureAggregate() {
  try {
    return JSON.parse(await readFile(resolve(repoRoot, phase597Group.sequenceEvidencePath), "utf8"));
  } catch {
    return null;
  }
}
...
async function writeTopReport(payload = report) {
  await writeFileWithRetry(resolve(repoRoot, phase597Group.sequenceEvidencePath), `${JSON.stringify(payload, null, 2)}\n`);
}
