...
import { phase595Group, phase595SafetyBoundary, phase595Subphases } from "./phase595-registry.mjs";
...
for (const phase of phase595Subphases) {
...
report.completed = report.phases.length === phase595Group.subphases.length && report.phases.every((item) => item.completed === true);
...
async function readClosureAggregate() {
  try {
    return JSON.parse(await readFile(resolve(repoRoot, phase595Group.sequenceEvidencePath), "utf8"));
  } catch {
    return null;
  }
}
...
async function writeTopReport(payload = report) {
  await writeFileWithRetry(resolve(repoRoot, phase595Group.sequenceEvidencePath), `${JSON.stringify(payload, null, 2)}\n`);
}
