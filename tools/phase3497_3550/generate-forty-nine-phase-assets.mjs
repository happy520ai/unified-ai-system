...
async function buildProposalDiff(target, beforeText, afterText) {
  ...
  const tempDir = resolve("tmp/phase3497-3550-proposals");
...
function buildDoc() {
  return `# Phase3497A-3550A Controlled Forty-Nine Tool Mutation
  ...
  return JSON.stringify(
    {
      ...
      planId: "phase3497-3550-controlled-forty-nine-tool-mutation",
      maxChangedFiles: 49,
      runNodeCheck: true,
      runLocalSmoke: true,
      approvalRecord: {
        ...
        approvedAtLeastOnceByOwner: [
          {
            ...
            scope: "phase3497-3550-controlled-forty-nine-tool-mutation",
          },
        ],
...
function buildBaseResult({ plan, validation, generatedAt, planPath }) {
  return {
    ...
    phase3496Sealed: validation.upstreamResults.some((entry) => entry.id === "${previousPhaseMeta.sealCheckId}" && entry.passed),
    codexExecExecuted: false,
    providerCallsMade: false,
    projectProviderCallsMade: false,
    secretRead: false,
...
function buildVerifierTemplate() {
  ...
  const packageSmoke = "node tools/phase3497_3550/smoke-controlled-forty-nine-tool-mutation.mjs";
...
const phase3496 = readJson("${previousPhaseMeta.resultPath}") || {};
...
check("${previousPhaseMeta.sealCheckId}", phase3496.recommendedSealed === true && phase3496.${previousPhaseMeta.sealCheckField} === true);
...
async function updatePackageScripts() {
  ...
  const smokeScript = "node tools/phase3497_3550/smoke-controlled-forty-nine-tool-mutation.mjs";
