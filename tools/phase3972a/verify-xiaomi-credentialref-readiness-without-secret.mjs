...
async function main() {
  console.log("[Phase3972A] Xiaomi CredentialRef Readiness Verifier");

  const resultPath = resolve(repoRoot, "apps/ai-gateway-service/evidence/phase-3972a-xiaomi-credentialref-readiness-without-secret/result.json");
  let result;
  try {
    result = JSON.parse(await readFile(resultPath, "utf8"));
  } catch {
    console.error("[Phase3972A] FAIL: result.json not found.");
    process.exit(1);
  }
  ...
  const checks = [
    ...
    { field: "xiaomiCredentialRefReadinessChecked", expected: true, actual: result.xiaomiCredentialRefReadinessChecked },
  ...
  console.log(`  INFO: readyForOwnerAuthorizedOneShotSmoke = ${result.readyForOwnerAuthorizedOneShotSmoke}`);
  console.log(`  INFO: blocker = ${result.blocker ?? "none"}`);
  if (result.details) {
    console.log(`  INFO: credentialRefName = ${result.details.credentialRefName}`);
    console.log(`  INFO: blockers = ${result.details.blockers?.join(", ") ?? "none"}`);
  }

  if (pass) {
    console.log("[Phase3972A] PASS: All checks passed.");
  } else {
    console.log("[Phase3972A] FAIL: Some checks failed.");
    process.exit(1);
  }
}
