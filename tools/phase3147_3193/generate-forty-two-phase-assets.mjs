...
async function buildProposalDiff(target, beforeText, afterText) {
  ...
  const tempDir = resolve("tmp/phase3147-3193-proposals");
...
function buildDoc() {
  return `# Phase3147A-3193A Controlled Forty-Two Tool Mutation
  ...
  return JSON.stringify(
    {
      ...
      planId: "phase3147-3193-controlled-forty-two-tool-mutation",
      maxChangedFiles: 42,
      runNodeCheck: true,
      runLocalSmoke: true,
      approvalRecord: {
        ...
        approvedAtLeastOnceByOwner: [
          {
            ...
            scope: "phase3147-3193-controlled-forty-two-tool-mutation",
          },
        ],
...
${smokeMarkers}
  ...
  const result = {
    ...
    blocker: completed ? "none" : "forty_two_mutation_node_check_or_smoke_failed",
...
function buildBaseResult({ plan, validation, generatedAt, planPath }) {
  return {
    ...
    phase3146Sealed: validation.upstreamResults.some((entry) => entry.id === "${previousPhaseMeta.sealCheckId}" && entry.passed),
    codexExecExecuted: false,
    providerCallsMade: false,
    projectProviderCallsMade: false,
    secretRead: false,
...
function printSummary(result) {
  console.log(JSON.stringify({
    status: result.status,
    blocker: result.blocker,
    fortyTwoMutationApplied: result.fortyTwoMutationApplied,
    changedFileCount: result.changedFileCount,
    nodeCheckPassed: result.nodeCheckPassed,
    localFortyTwoSmokePassed: result.localFortyTwoSmokePassed,
...
const phase3146 = readJson("${previousPhaseMeta.resultPath}") || {};
...
check("${previousPhaseMeta.sealCheckId}", phase3146.recommendedSealed === true && phase3146.${previousPhaseMeta.sealCheckField} === true);
