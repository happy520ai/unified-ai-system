...
import { phase594Group, phase594SafetyBoundary, phase594Subphases } from "./phase594-registry.mjs";
...
for (const phase of phase594Subphases) {
...
report.completed = report.phases.length === phase594Group.subphases.length && report.phases.every((item) => item.completed === true);
...
async function readClosureAggregate() {
  try {
    return JSON.parse(await readFile(resolve(repoRoot, phase594Group.sequenceEvidencePath), "utf8"));
  } catch {
    return null;
  }
}
...
async function writeTopReport(payload = report) {
  await writeFileWithRetry(resolve(repoRoot, phase594Group.sequenceEvidencePath), `${JSON.stringify(payload, null, 2)}\n`);
}
