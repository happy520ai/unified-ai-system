...
async function buildProposalDiff(target, beforeText, afterText) {
  ...
  const tempDir = resolve("tmp/phase2807-2845-proposals");
...
function buildDoc() {
  return `# Phase2807A-2845A Controlled Thirty-Four Tool Mutation
  ...
  return JSON.stringify(
    {
      ...
      planId: "phase2807-2845-controlled-thirty-four-tool-mutation",
      maxChangedFiles: 34,
      runNodeCheck: true,
      runLocalSmoke: true,
      approvalRecord: {
        ...
        approvedAtLeastOnceByOwner: [
          {
            ...
            scope: "phase2807-2845-controlled-thirty-four-tool-mutation",
          },
        ],
...
function buildBaseResult({ plan, validation, generatedAt, planPath }) {
  return {
    ...
    phase2806Sealed: validation.upstreamResults.some((entry) => entry.id === "${previousPhaseMeta.sealCheckId}" && entry.passed),
    codexExecExecuted: false,
    providerCallsMade: false,
    projectProviderCallsMade: false,
    secretRead: false,
...
const phase2806 = readJson("${previousPhaseMeta.resultPath}") || {};
...
check("${previousPhaseMeta.sealCheckId}", phase2806.recommendedSealed === true && phase2806.${previousPhaseMeta.sealCheckField} === true);
