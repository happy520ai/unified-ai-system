...
async function buildProposalDiff(target, beforeText, afterText) {
  ...
  const tempDir = resolve("tmp/phase2594-2626-proposals");
...
function buildDoc() {
  return `# Phase2594A-2626A Controlled Twenty-Eight Tool Mutation
  ...
  return JSON.stringify(
    {
      ...
      planId: "phase2594-2626-controlled-twenty-eight-tool-mutation",
      maxChangedFiles: 28,
      runNodeCheck: true,
      runLocalSmoke: true,
      approvalRecord: {
        ...
        approvedAtLeastOnceByOwner: [
          {
            ...
            scope: "phase2594-2626-controlled-twenty-eight-tool-mutation",
          },
        ],
...
function buildBaseResult({ plan, validation, generatedAt, planPath }) {
  return {
    ...
    phase2593Sealed: validation.upstreamResults.some((entry) => entry.id === "${previousPhaseMeta.sealCheckId}" && entry.passed),
    codexExecExecuted: false,
    providerCallsMade: false,
    projectProviderCallsMade: false,
    secretRead: false,
...
const phase2593 = readJson("${previousPhaseMeta.resultPath}") || {};
...
check("${previousPhaseMeta.sealCheckId}", phase2593.recommendedSealed === true && phase2593.${previousPhaseMeta.sealCheckField} === true);
