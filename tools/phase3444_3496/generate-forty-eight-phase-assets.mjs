...
async function buildProposalDiff(target, beforeText, afterText) {
  ...
  const tempDir = resolve("tmp/phase3444-3496-proposals");
...
function buildDoc() {
  return `# Phase3444A-3496A Controlled Forty-Eight Tool Mutation
  ...
  return JSON.stringify(
    {
      ...
      planId: "phase3444-3496-controlled-forty-eight-tool-mutation",
      maxChangedFiles: 48,
      runNodeCheck: true,
      runLocalSmoke: true,
      approvalRecord: {
        ...
        approvedAtLeastOnceByOwner: [
          {
            ...
            scope: "phase3444-3496-controlled-forty-eight-tool-mutation",
          },
        ],
...
function buildBaseResult({ plan, validation, generatedAt, planPath }) {
  return {
    ...
    phase3443Sealed: validation.upstreamResults.some((entry) => entry.id === "${previousPhaseMeta.sealCheckId}" && entry.passed),
    codexExecExecuted: false,
    providerCallsMade: false,
    projectProviderCallsMade: false,
    secretRead: false,
...
function buildVerifierTemplate() {
  ...
  const packageSmoke = "node tools/phase3444_3496/smoke-controlled-forty-eight-tool-mutation.mjs";
...
const phase3443 = readJson("${previousPhaseMeta.resultPath}") || {};
...
check("${previousPhaseMeta.sealCheckId}", phase3443.recommendedSealed === true && phase3443.${previousPhaseMeta.sealCheckField} === true);
...
async function updatePackageScripts() {
  ...
  const smokeScript = "node tools/phase3444_3496/smoke-controlled-forty-eight-tool-mutation.mjs";
